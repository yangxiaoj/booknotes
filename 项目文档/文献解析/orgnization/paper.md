...
本文描述了一个科学实验中的数据整合与表达分析的结果。实验中使用了单核苷酸RNA测序（snRNA-seq）和CODEX技术来对细胞类型进行分析和整合。研究中得到了92.1%的整合匹配精确度，这表示不同技术源自的相同细胞类型在共嵌入空间（co-embedding space）中能够很好地混合，并展示出一致的RNA和蛋白质表达模式。这显示了整合性能的稳健性(Fig. 5a)。

利用这种数据整合，研究者计算了之前由CODEX定义的细胞邻域中的差异表达基因（DEGs） (Extended Data Fig. 9f)。特别是，研究者检查了与滤泡邻域相关的DEGs的表达水平(Fig. 5a (bottom UMAP))，并识别了在这些空间组织中富集的基因通路，包括B细胞受体信号、T细胞极性调控以及自我抗原途径的耐受诱导(Extended Data Fig. 9g)。这些发现通过整合CODEX和snRNA-seq数据，揭示了与不同细胞邻域相关的特定表达模式。

图例:
- Fig. 5a

![](images/2024-05-14-19-39-23.png)

snRNA-seq 和 CODEX 集成使用MaxFuse 适用于小肠和结肠。集成细胞的颜色为来源方式、细胞类型或 RNA/蛋白质表达水平。 MUC2表达
（蛋白质和 RNA）显示在积分图上，以及前 10 个 DEG由 CODEX 确定的内卵泡邻域。最小值，最小值。CN，蜂窝邻里。

- Extended Data Fig. 9f

![](images/2024-05-14-19-44-43.png)


差异表达基因的热图（来自 MaxFuse 匹配的 snRNA-seq 细胞）在各个 CODEX 单元中，根据先前确定的分组来自 CODEX 分析的细胞邻域。


- Extended Data Fig. 9g

![](images/2024-05-14-19-46-26.png)

基因本体富集分析细胞邻域“内卵泡”和“外卵泡”，基于f中所示的差异表达基因。
...
想象一下，你正在组织一场大型音乐会，其中包括来自世界各地的各种音乐风格的乐队。这场音乐会的目标是要让所有的乐队都能和谐地共同演出，尽管它们各自拥有不同的音乐风格和表演方式。在这个比喻中，音乐会就像是我们的科学实验，各种音乐风格的乐队代表着不同的细胞类型，而来自世界各地的乐队则相当于使用不同技术（如snRNA-seq和CODEX）得到的细胞数据。

首先，你需要确保所有乐队都能在同一舞台上协调演出，这需要一个精心设计的舞台布局和音响系统，确保每个乐队的风格都得以体现，同时又能和其他乐队和谐地融合。这就类似于实验中的“共嵌入空间”，在这个空间中，来自不同技术的细胞数据被整合在一起，展示出一致的RNA和蛋白表达模式。

在音乐会中，你可能会特别关注某些乐队的表演，比如说一个特别的爵士乐团可能需要展示其独特的即兴演奏技巧。这就像在科学实验中特别分析某个细胞邻域（比如滤泡邻域）的差异表达基因，研究它们在整个细胞群体中的独特表达和功能。

最终，通过精确的音控和舞台管理，音乐会能够成功地展示每个乐队的特色，同时让所有观众享受到一个和谐且多元化的音乐盛宴。这正如通过有效的数据整合技术，科学家能够揭示细胞类型之间复杂的交流模式和功能路径，加深我们对生物系统多样性和复杂性的理解。
...
为了深入了解控制肠道细胞分化的因素，研究团队首先调查了可能调控肠道细胞类型中基因表达的转录因子（TFs）。通过计算数据集中每个细胞的ChromVar偏差得分，研究者识别了与不同细胞类型中染色质可访问性相关的TF基序。由于许多转录因子具有相似的基序，因此检查转录因子表达与基序活性结合使用，有助于确定在不同肠道细胞类型中功能性的具体转录因子。接下来，研究者识别了基因表达与其假定DNA结合基序的染色质可访问性活动水平之间相关性最高的转录因子，以推测直接推动可访问性变化的转录因子。在整个肠道中，此分析揭示了61个基序活性与表达强相关（相关系数r > 0.5）的转录因子。总体上，观察到活跃在分泌谱系的转录因子和活跃在吸收谱系的转录因子，很少有同时参与两者的转录因子（例如，KLF4在结肠上皮细胞和整个肠道的杯状细胞中）。

图例:

![](images/2024-05-14-19-56-41.png)

这段文字展示了肠道特定区域中一些转录因子的活性与其整合基因表达之间的相关性。左侧用行标准分显示染色体变异评分，右侧用行标准分显示整合转录因子表达。Ent. 代表肠系细胞，abs. 代表吸收性。
...
想象一下肠道就像一个精密的机器，这个机器中有各种各样的零件（细胞），每个零件都有其特定的功能。而控制这些零件如何工作和协同的是一套复杂的电路系统（转录因子）。这些电路系统通过接收外部信号来调节机器的运行，确保它能够根据需要高效地进行食物消化和营养吸收。

在这个比喻中，研究者们首先需要了解哪些电路（转录因子基序）在机器的不同部分（不同细胞类型）中被激活，他们通过检测每个细胞内电路的活动情况（ChromVar偏差得分），来识别与染色质可访问性相关的特定电路。染色质可访问性在这里可以被视为电路板上特定部分的电流强弱，直接影响机器的工作效率。

由于许多电路设计看起来非常相似（许多转录因子有相似的基序），研究者们需要检查这些电路是否真正连接并激活了机器的相应部分。他们通过分析这些电路的实际运作情况与预期工作状态的匹配程度（基因表达与染色质可访问性活动水平的相关性），找出那些真正调控机器运行的关键电路。

通过这样的方法，研究者发现了61种主要的电路设计，这些设计在确保机器（肠道）在消化过程中有效分泌和吸收（分泌谱系和吸收谱系的转录因子）方面发挥了至关重要的作用。特别是，某些电路（如KLF4）被发现不仅在机器的一个部分中起作用，在整个系统中都有其影响，使得整个肠道机器运行更为高效和协调。

这种深入的机制探索帮助研究者们更好地理解并最终改进这台复杂机器的性能，确保它能在各种条件下都能保持最佳运行状态。

*****

如何确定特定的转录因子在肠道细胞类型中的功能性影响？

答案和讲解：
为了确定特定的转录因子在肠道细胞类型中的功能性影响，研究者首先计算了每个细胞的ChromVar偏差得分，用以识别与染色质可访问性相关的转录因子基序。之后，他们通过比较这些转录因子的基因表达与其DNA结合基序的染色质可访问性活动水平之间的相关性，从而推断出哪些转录因子可能直接推动染色质状态的变化。通过这种方法，研究者能够挑选出与表达强烈相关的转录因子，这种相关性表明这些转录因子在调控肠道细胞类型中的基因表达方面具有实际的功能性作用。

比喻讲解：
想象一下，肠道细胞是一个有很多按钮和开关的控制面板，而转录因子就像是这些按钮和开关的背后电路。为了理解哪些按钮真正影响了控制面板的功能，科学家们首先需要检测每个按钮背后的电流活动（ChromVar偏差得分），确定哪些按钮是活跃的。然后，他们通过查看按下按钮时的实际效果（基因表达）与电流活动的关联，来确认哪些按钮不仅连接了电路，而且确实在控制面板上起到了调节作用。这样，他们就能够找出真正控制肠道细胞功能的关键按钮，即功能性转录因子。


*****

文本分析了多个在肠道中具有重要作用的转录因子（TF）。例如，ASCL2作为肠道干细胞的主要调节因子，在干细胞中表达量高且基序可达性高。其他在干细胞中表达和基序活性高的转录因子包括NFIX、NFIC和HNF1B。在分泌细胞系中，POU2F3是小鼠嗜瘤细胞发育所必需的调节因子，在整个人类肠道的嗜瘤细胞中表达量高且基序活性高。在比较嗜瘤细胞中POU2F3基序周围的转录因子足迹信号与肠上皮细胞时，发现嗜瘤细胞中POU2F3基序周围的可达性更高，这表明POU2F3更有可能在嗜瘤细胞中与其基序结合（扩展数据图10d）。除POU2F3外，RUNX1和RUNX2在整个肠道的嗜瘤细胞中也表现出高表达和高可达性，表明它们也可能在嗜瘤细胞的分化或维持中发挥作用。在杯状细胞中，KLF4是小鼠结肠杯状细胞末梢分化所需的，其基因表达和基序活性高。值得注意的是，KLF4的表达和基序活性在结肠的分化吸收上皮细胞（未成熟的肠上皮细胞和肠上皮细胞）中也很高，但在肠道的其他区域则不高，表明存在特定位置的调节。

图例:

![](images/2024-05-14-20-07-45.png)

TF 图案足迹近端空肠簇细胞和肠细胞中的 POU2F3。
...
想象一下肠道是一个忙碌的城市，城市中的每个细胞都是一个特定的居民，具有不同的职责和功能。在这个城市中，转录因子（TFs）就像是各个区域的市长和管理者，他们指导和调节这些居民（细胞）的行为和特性。

例如，ASCL2就像是住宅区的区长，它是肠道干细胞的主要管理者，确保这些干细胞能够维持其基本的活力和更新能力。ASCL2在干细胞中表达量高，就好比这位区长在其管辖的区域里拥有极高的影响力和可访问性，使得这个区域始终保持活力和生机。

同样，POU2F3则可以被看作是专门负责发展和维护城市中特定社区——tuft细胞社区的市长。在这些tuft细胞中，POU2F3的表达和活性都很高，说明它在这个社区中极具影响力，有如一个能够推动社区发展的领导者。而对比在其他类型细胞（如enterocytes肠上皮细胞）中的POU2F3基序周围的足迹信号，我们发现tuft细胞中POU2F3的活动区域更为开放，这就好比这位市长在自己的社区中拥有更多自由度和更大的权力范围。

再比如，RUNX1和RUNX2在tuft细胞中也显示出高表达和高可访问性，这就像是有其他几位官员也在同一个社区中担任关键角色，共同推动社区的发展和维护。

在另一个社区——goblet细胞社区，KLF4就像是负责该社区终端分化的市长，特别是在小鼠中的结肠goblet细胞。KLF4在这些细胞中的表达和活性都非常高，表明它在维持社区特色和功能中扮演着核心角色。而在结肠的成熟吸收性上皮细胞中，KLF4的表达和活性也同样高，显示出它在不同区域中可能扮演着类似或稍有不同的角色，但在肠道的其他区域则没有这样的情况，这说明它的影响力可能具有地点特异性。

这种管理模式确保了肠道这个“城市”能够根据不同区域的特定需求来进行有序的运转和发展，使得每个区域都能够发挥其最大的功能和效益。
...
在这段文本中，研究者们提出了一些在杯状细胞（goblet cells）中重要的调控转录因子（regulatory TFs），包括ATOH1和FOXA3。

1. **ATOH1**： (Extended Data Fig. 10e)
    - ATOH1在小鼠肠道中对于分泌谱系的承诺（secretory lineage commitment）是必需的。这里的数据支持了ATOH1在人体杯状细胞中有类似的功能。
    - 为了进一步证明ATOH1在杯状细胞中驱动了可及性（accessibility），研究者们比较了杯状细胞和肠细胞（enterocytes）中的ATOH1足迹（footprints）。
    - 结果显示，在杯状细胞中，ATOH1基序（motifs）周围的可及性更高，这表明ATOH1在这些细胞中的结合更强。

2. **FOXA3**：
    - 有证据表明，FOXA3在肺中导致杯状细胞化生（metaplasia）。
    - 研究者们的发现指出，FOXA3可能是结肠中一个重要的调控转录因子。

图例:

![](images/2024-05-14-20-27-00.png)

TF主题足迹十二指肠杯状细胞和肠上皮细胞中的 ATOH1。 d 和 e 中的误差带
代表标准差。
...
这段文本可以比喻为一个复杂的生态系统，其中不同的生物扮演着特定的角色，共同维持生态平衡。我们可以将杯状细胞（goblet cells）和肠细胞（enterocytes）比作生态系统中的不同动物，而ATOH1和FOXA3则是这些动物中的关键领导者，决定了生态系统的平衡和健康。

1. **ATOH1**：

- **ATOH1**就像是一个森林的护林员，他的职责是确保森林中某一特定种类的树木（比作分泌谱系）能够茁壮成长。在小鼠的肠道中，护林员ATOH1至关重要，因为没有他，这些树木就无法生长。

- 现在，研究者们在人体的“森林”中也发现了护林员ATOH1的作用。为了证明ATOH1在杯状细胞中的重要性，研究者们比较了“森林”的两个区域：杯状细胞区和肠细胞区。他们发现，在杯状细胞区，护林员ATOH1能够更自由地活动（更高的基序周围可及性），这说明ATOH1在这个区域起到了更重要的作用。

2. **FOXA3**：

- **FOXA3**则可以比作沙漠中的一种特殊植物，这种植物可以改变周围的环境，使得原本干燥的沙漠变成适合其他生物生存的绿洲。在肺部，这种植物FOXA3能引发杯状细胞的“化生”（metaplasia），类似于将干旱地区变成绿洲。

- 研究者们的发现进一步表明，FOXA3在结肠这个“生态系统”中也扮演着类似的角色，可能是一个重要的调控者，帮助维持结肠的健康。

通过这种比喻方式，我们可以更直观地理解ATOH1和FOXA3在细胞和基因调控中的关键作用。
...
在杯状细胞中，ATOH1和FOXA3的作用分别是什么？请解释它们在细胞中的具体功能，并指出研究者们是如何证明这些功能的。

答案和讲解：

1. **ATOH1**：

- **功能**：ATOH1在小鼠肠道中是分泌谱系承诺（secretory lineage commitment）所必需的。它在人体杯状细胞中也有类似的功能，驱动细胞的分泌功能。

- **证明方法**：研究者们通过比较杯状细胞和肠细胞中的ATOH1足迹（footprints），发现杯状细胞中ATOH1基序（motifs）周围的可及性更高，这表明ATOH1在杯状细胞中有更强的结合能力和作用。

2. **FOXA3**：

- **功能**：FOXA3在肺中可以导致杯状细胞的化生（metaplasia），并且在结肠中可能是一个重要的调控转录因子。

- **证明方法**：研究者们的发现指出了FOXA3在结肠中的重要调控作用，这表明FOXA3可能对杯状细胞的功能和分化起关键作用。

## 比喻讲解：

1. **ATOH1**：

- **比喻**：ATOH1像是森林的护林员，确保特定种类的树木（分泌谱系）能够生长。在小鼠和人体的“森林”中，护林员ATOH1都是必不可少的。在杯状细胞的“区域”中，护林员ATOH1更活跃，这说明他在这里的作用更重要。

- **证明方法**：通过比较不同区域（杯状细胞区和肠细胞区），发现护林员在杯状细胞区的活动范围更大（基序周围可及性更高），从而证明了ATOH1在杯状细胞中的重要性。

2. **FOXA3**：

- **比喻**：FOXA3像是沙漠中的特殊植物，可以将干旱的沙漠变成绿洲。在肺中，这种植物FOXA3能改变环境，使杯状细胞发生化生。在结肠中，FOXA3也可能起到类似的作用，调控细胞功能和分化。

- **证明方法**：研究者发现FOXA3在结肠中是一个重要的调控转录因子，类似于植物在沙漠中创造绿洲的过程，显示了FOXA3在杯状细胞调控中的重要作用。
...
我们注意到杯状细胞在结肠中比在小肠中更为丰富，但目前尚不清楚是什么因素导致细胞在结肠中更频繁地分化为杯状细胞。可能的原因包括隐窝中的信号传导差异和这两个区域之间干细胞的差异。我们识别了在小肠和结肠干细胞之间差异性峰值中富集的基序，并发现FOX基序在结肠干细胞中更易于获取的峰值中富集（扩展数据图10a–c）。同样，FOX转录因子的基序偏差分数在结肠干细胞中相比小肠干细胞更高（扩展数据图10a–c）。由于我们提名了几个FOX转录因子作为杯状细胞的潜在调节因子，增加的FOX活性可能部分解释了为什么杯状细胞在结肠中更为丰富。

图例:

扩展数据图10a–c

![](images/2024-05-14-20-50-50.png)


a) 超几何结肠干细胞之间差异峰中选定 TF 基序的 p 值以及来自肠道其他区域的干细胞。颜色代表log10使用 ArchR 计算调整后的 p 值。 b) 主题偏差分数的小提琴图FOXA3 用于杯状细胞、未成熟杯状细胞、干细胞和肠上皮细胞肠道的不同区域。 c) 表达的点图表示杯状细胞、未成熟杯状细胞、干细胞中的不同转录因子，以及肠道不同区域的肠细胞。

*****

比喻讲解：

想象一下，我们的消化系统就像一个巨大的果园，种满了各种各样的树木。在这个果园里，有两片主要的果园区域：一个是小果园（相当于小肠），另一个是大果园（相当于结肠）。在大果园里，我们发现一种特定的树木——杯状细胞树，比在小果园里要多得多。

现在，问题来了，为什么杯状细胞树在大果园里比在小果园里更多呢？这可能有两种原因：第一，可能是大果园的土壤和气候（隐窝中的信号传导）更适合这种树的生长；第二，可能是大果园里的种子（干细胞）与小果园里的种子不同，更容易长成杯状细胞树。

为了弄清楚其中的原因，科学家们做了一些实验，就像园艺师们仔细研究果园里的土壤和种子。他们发现，在大果园里的土壤中，有一种特别的养分——FOX基序（相当于特定的基因序列），这种养分在大果园的土壤中比在小果园的土壤中更多。而且，这些FOX基序对大果园里的种子有更大的影响力（基序偏差分数更高）。

科学家们由此推测，这些FOX基序就像是特别的肥料，帮助更多的种子长成了杯状细胞树。因此，FOX基序的增加可能部分解释了为什么大果园里的杯状细胞树比小果园里的多。

*****

问题：

为什么FOX转录因子的活性增加可能会导致结肠中杯状细胞数量比小肠中更多？

答案和讲解：

FOX转录因子的活性增加可能导致结肠中杯状细胞数量更多的原因在于，FOX基序在结肠干细胞中的富集程度更高，并且这些基序在结肠干细胞中的影响力（基序偏差分数）也更大。科学家们认为，这些FOX转录因子可能是杯状细胞的潜在调节因子。因此，FOX基序的增加可能促进了更多的细胞分化为杯状细胞，解释了为什么结肠中有更多的杯状细胞。

比喻讲解：

就像一个果园里，特定的肥料（FOX基序）能更好地促进某种树（杯状细胞树）的生长。如果这个肥料在大果园（结肠）的土壤里更多，而且对种子（干细胞）的影响力更大，那么就会有更多的种子长成这种树。这解释了为什么在大果园里会有更多这种特定的树。科学家认为，FOX转录因子就像这种特别的肥料，促进了杯状细胞在结肠中的生长。

*****



在肠内分泌细胞/肠嗜铬细胞中，RFX2、RFX3和RFX6表现出高表达和高可及性。其中，RFX6被认为是肠内分泌细胞分化的调节因子，缺失RFX6会损害小鼠肠内分泌细胞的分化。这些结果支持了之前的研究结果，并提名了其他可能是重要调节因子的转录因子，这些转录因子可能在不同肠道区域的不同细胞类型中起重要作用，例如KLF4。
...
可以把肠内分泌细胞/肠嗜铬细胞想象成一个大型工厂，这个工厂需要一些特殊的工人来完成特定的任务。在这个工厂里，RFX2、RFX3和RFX6就像是工厂的经理，他们在工厂里有着重要的职位，并且很容易被找到。

其中，RFX6这个经理特别重要，因为它负责培训和指导新来的工人，使他们能够胜任肠内分泌细胞的工作。如果这个RFX6经理不在了，新工人就不知道该怎么工作，整个工厂的运行就会受到影响。

这些研究结果不仅支持了之前的发现，还提到了其他可能的重要经理（比如KLF4），这些经理可能在工厂的不同部门中发挥重要作用，确保各个部门都能正常运转。
...
为什么RFX6在肠内分泌细胞分化中起着重要作用？如果RFX6缺失会发生什么？

### 答案和讲解：

RFX6在肠内分泌细胞分化中起着重要作用，因为它是这些细胞分化的一个关键调节因子。研究表明，RFX6的高表达和高可及性对肠内分泌细胞的正常分化至关重要。如果RFX6缺失，这些细胞将无法正常分化，从而导致肠内分泌细胞分化受损。

### 比喻讲解：

RFX6就像是工厂里负责培训新工人的经理。如果这个经理不在了，新工人就不知道该怎么工作，整个工厂的运行就会受到影响。同样地，如果RFX6缺失，肠内分泌细胞将无法正常分化，导致肠道功能受损。

*****

### 讲解：

在这段文本中，研究者为了验证他们的发现，进行了两个独立队列的分析：multiome样本和分别含有snRNA和snATAC数据的样本。为了研究后者中的转录因子（TF）表达，研究者使用了经典相关分析（canonical correlation analysis, CCA）将snRNA和snATAC数据整合起来，目的是对齐数据集并将每个snATAC细胞分配给对应的snRNA数据。这个分析再现了多组学（multiome）分析中的许多发现，原本在多组学分析中识别的61个转录因子中有48个在此分析中达到了相同的显著性标准。

### 图例:

![](images/2024-05-15-12-31-02.png)

发现上皮调节因子的重叠具有多组数据以及单独的 snRNA 和 snATAC 数据集。

*****

### 比喻讲解：

想象有两个不同的乐队，每个乐队都有自己的乐谱和乐器。第一个乐队是一个综合乐队（multiome样本），他们的乐谱和乐器已经完美地协同工作，演奏出一首复杂的交响曲。第二个乐队由两个独立的小组组成：一个是弦乐组（snRNA数据），另一个是打击乐组（snATAC数据）。这两个小组的乐谱虽然不同，但他们想要一起演奏出同样的交响曲。

为了让这两个小组协调一致，我们使用了一种称为经典相关分析（CCA）的方法，这就像是一位指挥家，他能够看到并理解两个小组的乐谱，并指挥他们使其演奏同步。这位指挥家把弦乐组和打击乐组的乐谱进行了对齐，让每一个弦乐手都对应一个打击乐手，以便他们能一起演奏。

经过这种整合和对齐，我们发现第二个乐队（独立的小组）能够演奏出与第一个综合乐队非常相似的交响曲。原本在综合乐队中发现的61个和谐音符（转录因子）中，有48个在这两个小组的联合演奏中也达到了相同的效果。这表明两个乐队在很多方面是相似的，验证了我们的最初发现。

*****

### 问题：

在验证转录因子表达的研究中，研究者如何整合独立的snRNA和snATAC数据，并且这种整合分析结果是如何与multiome分析结果进行比较的？

### 答案和讲解：

研究者通过使用经典相关分析（CCA）方法将独立的snRNA和snATAC数据整合在一起。具体来说，他们使用CCA将这两种不同类型的数据对齐，并将snRNA数据分配给每一个snATAC细胞，从而创建了一个统一的数据集。通过这种方式，研究者可以在这些整合数据中分析转录因子的表达情况。最终，他们发现这种整合分析再现了许多在multiome分析中得到的结果。在61个转录因子中，有48个在整合分析中达到了与multiome分析相同的显著性标准，表明两个分析的结果具有高度一致性。
...
为了确定哪些转录因子（TFs）可能在肠道中的基质细胞和免疫细胞中驱动细胞功能，我们对这些隔室中的细胞进行了相同的分析。在免疫隔室内，这项分析强调了许多已知对细胞类型分化和维持重要的转录因子，包括在肥大细胞中的GATA2和在B细胞中的PAX5（扩展数据图10g）。在基质细胞中，我们鉴定了与特定谱系相关的转录因子，包括在周细胞中的EBF1（最近被建议对周细胞的承诺有贡献），在脂肪细胞中的PPARG和在神经胶质细胞中的SOX10（扩展数据图10h）。我们还提名了在肠间质细胞中的潜在调控转录因子，包括HAND1、HOXD11和MEIS1，以及不同类别的肠道成纤维细胞的潜在调控转录因子。

### 图例:

扩展数据图10g，扩展数据图10h

![](images/2024-05-15-12-34-33.png)

(g, h) 热图其整合基因表达为的转录因子的代表与其在肠道某一区域的免疫基序活性相关(g) 和基质 (h) 细胞类型。 ChromVar 偏差分数的行 z 分数为
显示在左侧，集成 TF 表达的行 z 分数显示在正确的。

*****

### 比喻讲解：

为了找出哪些转录因子（TFs）在肠道中的基质细胞和免疫细胞中起作用，就像在一个大型工厂中找出哪些工头（TFs）负责管理不同的部门。我们对工厂里的每个部门进行了相同的分析。

在免疫部门（相当于工厂里的安全部门），这项分析突出了许多已知的重要工头，比如GATA2就像负责肥大细胞的安全主管，而PAX5则像是管理B细胞的安全主管（参见扩展数据图10g）。

在基质细胞部门（相当于工厂里的维护和支持部门），我们找到了与特定工作类型相关的工头。例如，EBF1就像是周细胞（工厂的支持结构）的主管，最近有人建议他对周细胞的承诺有贡献；PPARG就像是管理脂肪细胞（工厂的能量储存部分）的主管，而SOX10则像是神经胶质细胞（工厂的信息传递部分）的主管（参见扩展数据图10h）。

此外，我们还提名了在肠间质细胞（相当于工厂的基础设施）的潜在工头，包括HAND1、HOXD11和MEIS1，以及不同类别的肠道成纤维细胞的潜在工头。
...
如何通过分析确定肠道中基质细胞和免疫细胞的功能驱动转录因子？

### 答案和讲解：

为了确定肠道中基质细胞和免疫细胞的功能驱动转录因子，研究人员对这些细胞进行了详细的分析。他们发现了一些已知对细胞类型分化和维持重要的转录因子。在免疫细胞中，GATA2在肥大细胞中起重要作用，而PAX5在B细胞中起重要作用。在基质细胞中，他们发现了与特定细胞谱系相关的转录因子，例如在周细胞中的EBF1，在脂肪细胞中的PPARG，以及在神经胶质细胞中的SOX10。此外，他们还发现了一些潜在的调控转录因子，这些转录因子可能在肠间质细胞和肠道成纤维细胞中起重要作用。
...
文本讲述了肠道干细胞分化为成熟肠细胞、杯状细胞及一些特殊细胞类型（如肠内分泌细胞、顶端细胞和帕内特细胞）的过程。这些细胞大约每三到七天更新一次肠上皮内层。为了描绘干细胞分化为成熟肠细胞过程中调控和基因表达的变化，研究人员在十二指肠、空肠、回肠和结肠中使用单核数据定义了分化轨迹（见图6a和方法部分）。

接下来，他们识别了这四种分化轨迹中染色质可及性（峰）和基因表达的可变区域。一个可变基因的例子是TMPRSS15，该基因编码在十二指肠中将胰蛋白酶原转化为胰蛋白酶的蛋白；其表达在更为分化的细胞中逐渐增加（见扩展数据图10m–p）。研究人员对这些可变峰和基因进行聚类，以识别出具有相似行为的基因集合（见图6b,c）。

### 图例:

- 图6a

![](images/2024-05-15-15-50-39.png)

热图描绘肠道四个主要区域细胞的投影（十二指肠、空肠、回肠和结肠），按细胞类型标记（左）和微分伪时间（右）

- 扩展数据图10m–p

![](images/2024-05-15-15-52-05.png)

(m–o) 整合基因表达MTTP (m)、SCNN1B (n) 和 LGR5 (o) 沿着分化轨迹。(p) 与 TMPRSS15 表达相关的峰的可达性每个区域的分化轨迹绘制在左侧。每个峰值是标准化为沿任何轨迹的最大可达性。融合的TMPRSS15 的基因表达沿着每个细胞的分化轨迹区域绘制在右侧。

- 图6b,c

![](images/2024-05-15-15-54-03.png)


b,c，可变峰 (b) 和基因(c) 沿着吸收分化轨迹识别。行数表示每个峰或每个表达的可及性 z 分数基因。这些列表示从开始到结束的伪时间位置
肠的每个部分的末端。峰和基因进行 k 均值聚类并根据主要时间和位置对集群进行标记他们最容易接触/表达的地方。
...
可以把肠道干细胞想象成一座工厂，这座工厂每天都在生产各种不同的产品。这些产品就是肠道中的各种细胞类型，如成熟的肠细胞、杯状细胞、肠内分泌细胞、顶端细胞和帕内特细胞。就像工厂每天都要运出新产品一样，这些细胞每三到七天就会更新一次肠上皮内层。

为了更好地理解工厂生产过程中发生的变化，研究人员就像是工厂的质量控制员，他们需要跟踪每一个生产阶段。他们在不同的车间（即十二指肠、空肠、回肠和结肠）中收集了生产数据，以便绘制出细胞从干细胞分化到成熟细胞的“生产流程图”（见图6a和方法部分）。

接下来，他们就像检测原材料的质量一样，识别了这些生产过程中发生变化的“原材料”（即染色质可及性和基因表达）的变化区域。例如，TMPRSS15基因就像是一种特定的原材料，它在十二指肠中逐渐增加，用于生产将胰蛋白酶原转化为胰蛋白酶的蛋白（见扩展数据图10m–p）。

最后，研究人员对这些变化的“原材料”进行分类和聚类，找出具有相似行为的“原材料”集合（见图6b,c），就像工厂中的不同生产线在不同阶段使用不同的原材料一样。
...
肠道干细胞分化过程中，研究人员是如何识别出具有相似行为的基因集合的？请描述他们的方法和一个具体的基因例子。

### 答案和讲解：

研究人员首先定义了肠道干细胞分化为成熟肠细胞的分化轨迹，这些轨迹覆盖了十二指肠、空肠、回肠和结肠的不同区域（见图6a和方法部分）。然后，他们识别出这些分化轨迹中染色质可及性和基因表达的可变区域。这些可变区域被称为“峰”和“可变基因”。

一个具体的基因例子是TMPRSS15基因。TMPRSS15编码在十二指肠中将胰蛋白酶原转化为胰蛋白酶的蛋白。研究发现，TMPRSS15基因的表达在更为分化的细胞中逐渐增加（见扩展数据图10m–p）。研究人员通过聚类分析将这些具有相似行为的可变峰和基因进行分类，从而识别出一组具有共同特性的基因集合（见图6b,c）。

### 比喻讲解：

可以把肠道干细胞的分化过程想象成一个工厂的生产流程。研究人员首先绘制了生产流程图，标明了不同的车间（即十二指肠、空肠、回肠和结肠）的生产阶段（见图6a和方法部分）。接下来，他们就像质量检测员一样，找出了在这些生产阶段中变化的原材料（即染色质可及性和基因表达的可变区域）。

一个具体的例子是TMPRSS15基因，就像是一种在生产过程中使用的特定原材料。研究发现，这种原材料在生产流程的后期（即更为分化的细胞中）逐渐增多（见扩展数据图10m–p）。最后，研究人员对这些变化的原材料进行分类和聚类，就像是将具有相似性质的原材料归类为同一组，从而识别出一组具有共同特性的原材料集合（见图6b,c）。

*****



文中描述了在分化拟时间（differentiation pseudotime）过程中，不同细胞群体中的基因和染色质开放区域的变化情况。这些变化被划分为不同的簇（clusters），每个簇包含在肠道各个区域中表达和开放的基因及染色质区域。

1. **早期簇（early cluster）**：这些簇包括在分化初期（如干细胞阶段）在所有肠道区域中开放和表达的基因。这些基因是肠道干细胞的一般标志物，包括RGMB、SOX9、SMOC2和LGR5。

2. **其他基因和峰值的簇**：

- **小肠分化细胞簇**：主要在分化的小肠细胞中发现，包括MTTP、APOA4、APOC3和MME。

- **未分化结肠簇**：主要在未分化的结肠细胞中发现，例如GPX2。

- **回肠簇**：在回肠中发现的基因，如PLB1和CUBN。

- **分化结肠簇**：在分化的结肠细胞中发现的基因，如SCNN1B、SLC26A3和CLCA4。
...
可以把肠道分化的过程想象成一棵大树的成长过程。树从种子开始，逐渐长成大树，而树的不同部分在不同的时间点会有不同的特点和功能。

1. **早期簇（early cluster）**：就像树的种子和幼苗阶段。这一阶段，种子开始发芽并且长出第一批叶子，所有的树叶都是基本的、相似的，代表了肠道中的干细胞。在这一阶段，树叶的标志特征包括基本的树木基因，如RGMB、SOX9、SMOC2和LGR5。

2. **其他基因和峰值的簇**：
   
- **小肠分化细胞簇**：像树长到一定高度，开始长出不同的树枝和树叶。这些树枝和树叶有着特定的功能和特点，就像在分化的小肠细胞中出现的基因（MTTP、APOA4、APOC3、MME）。这些基因类似于小肠中不同功能的树叶。
- **未分化结肠簇**：像树的一些树枝还未完全长出叶子，这些树枝是未分化的结肠细胞，具有特定的标志（如GPX2）。
- **回肠簇**：就像树的一部分专门长出一些特别的叶子，这些叶子有着特定的功能，这就类似于回肠中发现的基因（PLB1和CUBN）。
- **分化结肠簇**：像树的某些部分已经长成，树叶成熟并且具有特定的功能和特点，这些成熟的树叶就像分化的结肠细胞中的基因（SCNN1B、SLC26A3、CLCA4）。

总的来说，肠道的分化过程就像树的成长过程，不同的阶段和部分都有其特定的功能和标志。
...
在肠道分化的过程中，不同基因和染色质开放区域是如何在不同的肠道区域和分化阶段中表现出来的？请举例说明各个区域中典型的基因。

### 答案和讲解：

在肠道分化过程中，不同基因和染色质开放区域表现出特定的时空分布特征。具体表现如下：

1. **早期分化阶段（early cluster）**：这些基因在所有肠道区域的早期（例如在干细胞中）表达和开放。典型基因包括RGMB、SOX9、SMOC2和LGR5，这些都是肠道干细胞的标志物。

2. **小肠分化细胞**：在分化的小肠细胞中表达的基因包括MTTP、APOA4、APOC3和MME。这些基因主要在分化的小肠细胞中找到，表示这些细胞已经具有特定的功能。

3. **未分化结肠**：在未分化的结肠细胞中主要表达的基因包括GPX2。这个基因在未分化的结肠中表现活跃，说明这些细胞尚未完全成熟。

4. **回肠**：在回肠中表达的基因如PLB1和CUBN。这些基因特异性地在回肠中活跃，标志着回肠细胞的特征。

5. **分化结肠**：在分化的结肠细胞中主要表达的基因包括SCNN1B、SLC26A3和CLCA4。这些基因在成熟的结肠细胞中活跃，代表这些细胞已经完全分化并具有特定功能。

### 比喻讲解：

可以将肠道分化过程比作一棵大树的成长过程：

1. **早期分化阶段**：像树的种子和幼苗阶段，所有的树叶（基因）都是基础的、相似的，这些叶子（基因）是肠道干细胞的标志，包括RGMB、SOX9、SMOC2和LGR5。

2. **小肠分化细胞**：像树长到一定高度，开始长出不同的树枝和树叶，这些树枝和树叶有特定的功能，代表小肠中分化的细胞，典型的基因包括MTTP、APOA4、APOC3和MME。

3. **未分化结肠**：像树的一些树枝还未完全长出叶子，代表未分化的结肠细胞，主要基因是GPX2。

4. **回肠**：像树的一部分长出特别的叶子，有特定的功能，代表回肠中的细胞，典型基因是PLB1和CUBN。

5. **分化结肠**：像树的某些部分已经长成，树叶成熟，代表分化的结肠细胞，主要基因包括SCNN1B、SLC26A3和CLCA4。

*****

### 讲解：

为了识别每个簇中特定调控的染色质驱动因素和相关的簇特异性基因表达程序，我们计算了每个峰簇中的转录因子(TF)基序富集（Extended Data Fig. 10k）和每个基因簇中的KEGG通路富集（Extended Data Fig. 10l）。在分化轨迹后期可访问的峰群中，富集了HNF4和JUN/FOS基序。正如预期的那样，在小肠分化轨迹后期（后期十二指肠空肠簇）主要表达的基因富集了多个代谢KEGG通路，包括脂肪消化和吸收以及维生素消化和吸收。

### 图例:

Extended Data Fig. 10k, Extended Data Fig. 10l

![](images/2024-05-15-16-03-43.png)
![](images/2024-05-15-16-05-15.png)

k) 超几何b 中确定的峰簇中富集的 TF 基序的 p 值。 l) 浓缩c. 中确定的基因簇中的 KEGG 通路。未校正的 p 值由 kegga 确定并绘制。
...
要理解这段文字中的研究，我们可以将其比喻成一个复杂的园艺项目，其中我们希望识别哪些因素影响不同植物的生长方式。

1. **寻找特定园区的“花园设计师”**：

我们首先要找出哪些“花园设计师”（染色质驱动因素）在不同的花园（簇）中起作用。每个花园都有其独特的设计风格和布局，我们通过研究每个花园中的设计元素（转录因子基序富集）来找出哪些设计师在这些花园中做出了贡献。

2. **识别特定园区的“植物种类”**：

接下来，我们需要了解每个花园中种植了哪些种类的植物（基因表达程序）。每种植物都有其特定的生长需求和特性，我们通过分析每个花园中的植物种类（KEGG通路富集）来了解这些植物的特点和生长要求。

3. **发现晚期出现的花园设计师和植物种类**：

在园艺项目的后期，我们发现某些设计师（HNF4和JUN/FOS基序）和植物（特定基因）在某些花园中变得特别活跃。这些设计师在这些花园中实施了复杂的设计，而这些植物也在这些花园中得到了特别的关注和培育。

4. **特定花园的特殊种植**：

例如，在某些特定的花园（如后期的十二指肠空肠簇），我们发现了许多与代谢相关的植物（基因），这些植物特别擅长处理和吸收“营养物质”（脂肪消化和吸收，维生素消化和吸收）。

通过这种园艺项目的比喻，我们能够更好地理解这些复杂的生物信息学分析是如何帮助我们揭示不同细胞簇中特定的调控机制和基因表达程序的。
...
在研究特定基因簇的表达和调控时，研究人员如何识别关键的转录因子和基因通路？并解释在分化轨迹后期发现的富集转录因子和基因通路的意义。

### 答案和讲解：

研究人员通过计算每个峰簇中的转录因子(TF)基序富集和每个基因簇中的KEGG通路富集来识别关键的转录因子和基因通路。在分化轨迹后期，某些峰群富集了HNF4和JUN/FOS基序，这表明这些转录因子在该阶段具有重要的调控作用。同时，晚期在小肠（后期十二指肠空肠簇）主要表达的基因富集了多个代谢相关的KEGG通路，例如脂肪消化和吸收以及维生素消化和吸收。这表明在小肠分化的后期，基因表达和调控发生了显著变化，以适应特定的生理功能需求。

### 比喻讲解：

研究人员就像园艺师，试图了解每个花园（基因簇）的设计师（转录因子）和植物（基因）的种类。他们通过分析花园的设计元素（转录因子基序）和植物类型（KEGG通路），找出哪些设计师在特定花园中起作用，并识别出在特定花园中种植的植物种类。在分化的后期，他们发现某些设计师（HNF4和JUN/FOS）在花园中变得活跃，而这些花园中种植的植物则擅长处理特定的营养物质（脂肪和维生素的消化和吸收）。

*****

### 讲解：

在这段文本中，研究人员识别了在四种分化轨迹中基因表达与其基序活性相关的转录因子（TFs）。他们发现有67个TFs的表达与其基序活性相关（相关系数r > 0.5），并绘制了这些TFs的综合基因表达图（Fig. 6d）。许多TFs在四种分化轨迹中显示出相似的活性。例如，ASCL2是肠干细胞中的主要调控因子，在所有四种轨迹的开始阶段都高度表达。其他的TFs，如ETV6，在肠的不同区域表现出不同的行为。ETV6在结直肠癌中的表达低于正常结肠，并且ETV6的基因变异可能赋予结直肠癌的易感性。研究发现，ETV6在结肠中的表达高于小肠，并且在结肠中，ETV6的表达在更分化的细胞中增加，而在小肠中则相反。

### 图例:
Fig. 6d

![](images/2024-05-15-16-10-41.png)

整合基因表达其表达与 ChromVar 基序活性相关的 TF的分化轨迹。

*****

### 问题：

在研究四种分化轨迹中的转录因子时，研究人员发现了哪些关键的差异和相似性？请具体说明ASCL2和ETV6在这些分化轨迹中的表现。

### 答案和讲解：

研究人员发现了67个转录因子的表达与其基序活性相关（相关系数r > 0.5），并绘制了这些转录因子的综合基因表达图（Fig. 6d）。其中，ASCL2和ETV6是两个表现出显著差异和相似性的转录因子。

**ASCL2：**
ASCL2在所有四种分化轨迹的开始阶段都高度表达，显示出相似的活性。这表明ASCL2在不同分化路径的早期阶段都起到了重要的调控作用。

**ETV6：**
ETV6在不同区域的肠道中表现出不同的行为。在结肠中，ETV6的表达高于小肠，并且在结肠的更分化细胞中，其表达增加。而在小肠中，ETV6的表现恰恰相反。此外，ETV6在结直肠癌中的表达低于正常结肠，且其基因变异可能与结直肠癌的易感性相关。

### 比喻讲解：

想象一个大型音乐会，有许多乐队在不同的舞台上演出，每个舞台代表一种分化轨迹。研究人员发现了67个乐队的表演和观众反应之间有强烈的关联（相关系数r > 0.5），并绘制了一张综合图（Fig. 6d），展示了这些乐队在所有舞台上的表现。

**ASCL2：**
ASCL2这支乐队在所有舞台的开场部分都表演得非常出色，显示了相似的活跃度。这表明ASCL2在所有舞台的开场时都扮演了重要角色。

**ETV6：**
ETV6这支乐队在不同舞台上的表现有所不同。在结肠舞台上，ETV6的表演比在小肠舞台上更引人注目，并且在结肠舞台上更成熟的表演环节中表现得更好，而在小肠舞台上却相反。ETV6在结直肠癌的舞台上的表现不如在正常结肠的舞台上出色，且其基因变异可能影响它们在结直肠癌舞台上的表现。
...
在该文本中，研究者们调查了哪些调控元件可能负责ETV6基因在肠道不同区域中表达的变化。具体而言，他们通过分析肠道不同区域中的ETV6表达和相关的开放染色质峰值来研究这些基因的调控机制。

1. **ETV6表达的调控元件**：

- 研究者首先确定了与ETV6表达相关的染色质开放性峰值。这些峰值沿着肠道的分化伪时间（differentiation pseudotime）显示出不同的开放性。
    
2. **小肠中的情况**：

- 在小肠中，与ETV6表达相关的峰值主要位于外显子1（exonic 1）和启动子1（promoter 1）区域。这些峰值在分化轨迹上逐渐变得不那么开放，与ETV6在分化伪时间中表达减少的趋势一致。
    
3. **结肠中的情况**：

- 类似的行为也在结肠中观察到。这些相关峰值在分化轨迹上同样变得不那么开放，说明在结肠的分化过程中ETV6表达也是逐渐减少的。

通过这些观察，研究者能够推测出特定调控元件在肠道不同区域和不同分化状态下对ETV6表达的影响。

### 图例:
Fig. 6e

![](images/2024-05-15-16-15-46.png)

与相关的峰值处的可达性ETV6 沿着每个区域的分化轨迹的表达（左）。每个峰都被归一化为沿任意方向的最大可达性轨迹。右图，ETV6 沿着分化的整合基因表达每个区域的轨迹绘制在右侧。规范，规范化。
...
想象你有一个大型图书馆，里面有不同的书架，每个书架代表肠道的不同区域。图书馆的管理系统决定了哪些书（基因）在不同的书架上可以被读者（细胞）更容易地看到和阅读（表达）。

1. **ETV6表达的调控元件**：
- 就像图书馆的管理系统能够调整书架上的书籍展示位置，研究者找到了与ETV6基因表达相关的"展示位置"（染色质开放性峰值）。这些展示位置决定了读者在不同区域和时间段能否容易地看到这些书。

2. **小肠中的情况**：
- 在小肠这个书架上，ETV6这本书最初被放在一个显眼的位置（外显子1和启动子1区域），很容易被读者发现。但是，随着时间推移，这本书逐渐被移到一个不那么显眼的位置（开放性降低），就像分化伪时间中ETV6表达减少一样，读者越来越难以找到这本书。

3. **结肠中的情况**：
- 结肠这个书架上也发生了类似的情况。ETV6这本书一开始在显眼的位置，随着时间的推移，也逐渐被移到不显眼的位置（开放性降低），说明在结肠的分化过程中ETV6表达也逐渐减少。

通过这个图书馆的比喻，我们可以理解，肠道不同区域的"管理系统"（调控元件）决定了ETV6这本书在不同位置和时间段的展示情况，从而影响了它的可读性（表达水平）。

*****

### 问题：

哪些调控元件与ETV6基因在肠道不同区域中的表达变化有关？请解释它们在小肠和结肠中的行为。

### 答案和讲解：

**答案**：
ETV6基因在肠道不同区域中的表达变化与染色质开放性峰值（accessibility peaks）有关。在小肠中，与ETV6表达相关的峰值主要位于外显子1（exonic 1）和启动子1（promoter 1）区域；在结肠中，也观察到了类似的峰值。

**讲解**：
研究者发现，ETV6基因在肠道不同区域的表达受到特定染色质开放性峰值的调控。在小肠，这些峰值（外显子1和启动子1）在分化伪时间的过程中变得不那么开放，导致ETV6基因的表达逐渐减少。相似的情况在结肠中也观察到，说明这些调控元件在肠道的不同区域和分化状态下对ETV6基因的表达都有影响。

### 比喻讲解：

想象一个大型图书馆，图书馆的管理系统决定了哪些书（基因）在不同的书架（肠道不同区域）上更容易被读者（细胞）看到和阅读（表达）。在小肠的书架上，ETV6这本书最初放在显眼的位置（外显子1和启动子1区域），但随着时间推移，这本书逐渐被移到不显眼的位置，读者越来越难找到它。结肠的情况类似，ETV6这本书也逐渐被移到不显眼的位置，说明在不同区域和时间段内，管理系统（调控元件）对这本书的展示（表达）都有所影响。
...
在这段文本中，作者讨论了不同细胞分化过程中调控元件的可及性变化，以及这些变化如何影响基因表达。

1. **调控元件可及性变化**：作者指出，在结肠中，某些峰值的可及性在更分化的细胞中有所增加。他们推测这些调控元件可能驱动了ETV6基因在分化的结肠细胞中的高表达。
   
2. **调控元件识别方法**：作者提到可以使用同样的逻辑来识别在整个肠道中可能驱动基因表达变化的调控元件。例如，他们识别出了三个峰值，这些峰值与TMPRSS15基因的表达高度相关，可能驱动了该基因在十二指肠中的高表达（扩展数据图10p）。

3. **整体分析**：通过这种分析，作者提供了一个参考框架，用于理解干细胞向肠细胞分化过程中的调控机制。

### 图例: 

![](images/2024-05-15-16-20-46.png)

峰的可达性与 TMPRSS15 的表达相关每个区域的分化轨迹绘制在左侧。每个峰值是标准化为沿任何轨迹的最大可达性。融合的TMPRSS15 的基因表达沿着每个细胞的分化轨迹区域绘制在右侧。
...
可以把文本中的内容想象成一个乐团的表演，其中每个基因和调控元件就像是乐团中的乐器和乐谱。乐团在不同的演出场所（即不同的肠道部分）中演奏不同的乐曲（即基因表达）。

1. **调控元件可及性变化**：就像在一个音乐会中，不同的曲目在不同的场地中有不同的效果。例如，在结肠这个场地中，有一些乐谱（调控元件）的可见性和重要性在更复杂的曲目（更分化的细胞）中变得更加突出。这些乐谱使得某些乐器（如ETV6基因）在这个特定的场地中演奏得更加响亮和重要。

2. **调控元件识别方法**：就像指挥家能够识别出哪些乐谱在特定场地中可以让某些乐器演奏得更好，科学家也可以使用同样的逻辑来找到在肠道其他部分可能驱动基因表达变化的调控元件。例如，指挥家识别出了三份乐谱，这些乐谱在十二指肠这个场地中能让某个乐器（TMPRSS15基因）演奏得更好（扩展数据图10p）。

3. **整体分析**：通过这种方式，指挥家（科学家）能够为不同的演出场所（肠道各部分）编排出一场完整的音乐会（基因表达调控），从而理解乐团的每个乐器（基因）在不同场地中的作用和表现（干细胞向肠细胞分化过程中的调控机制）。

*****

### 问题：

在肠道中，不同部位的细胞分化过程中，如何识别并理解调控元件对基因表达的影响？请具体说明ETV6和TMPRSS15基因的例子。

### 答案和讲解：

在肠道的不同部位，细胞的分化过程受到特定调控元件的影响，这些调控元件的可及性变化能够驱动基因表达的改变。识别这些调控元件的步骤如下：

1. **识别调控元件**：通过分析基因组中调控元件的峰值可及性变化，可以找到哪些调控元件在细胞分化过程中变得更加活跃。例如，在结肠中，有些调控元件的可及性在分化的细胞中增加。

2. **关联基因表达**：将这些调控元件与基因表达数据结合起来，寻找高度相关的调控元件和基因。例如，某些调控元件与ETV6基因的高表达相关联，表明这些调控元件可能在分化的结肠细胞中驱动了ETV6的表达。

3. **验证其他部位**：使用相同的方法，可以在肠道的其他部位找到类似的调控元件。例如，研究发现有三个调控元件与TMPRSS15基因的高表达高度相关，这些调控元件可能驱动了TMPRSS15在十二指肠中的表达（扩展数据图10p）。

### 比喻讲解：

可以把这个过程比作一个乐团在不同场地演出，不同场地（肠道不同部位）需要不同的乐谱（调控元件）来演奏不同的乐曲（基因表达）。

1. **识别调控元件**：在结肠这个场地，一些乐谱（调控元件）变得更重要，使得某些乐器（基因）如ETV6能够更好地演奏（高表达）。

2. **关联基因表达**：通过观察哪些乐谱与乐器演奏效果（基因表达）相关，可以确定哪些乐谱在驱动特定乐器的演奏。

3. **验证其他部位**：在十二指肠这个场地，也可以通过识别特定乐谱，发现哪些乐谱使得特定乐器（如TMPRSS15）演奏得更好（高表达）。

*****

### 讲解：

该研究利用连锁不平衡得分回归（linkage-disequilibrium score regression）方法测试了肠道细胞类型中特异性标记峰（marker peaks）上的疾病遗传力是否富集（enriched）。主要结论如下：

1. **T细胞标记峰与克罗恩病和乳糜泻的遗传力显著增加**：
- 在T细胞标记峰中，克罗恩病和乳糜泻的遗传力显著增加。
- 这与T细胞在这些疾病发病机制中的重要性一致。

2. **BMI在肠内分泌细胞中的遗传力富集**：
- 观察到BMI在肠内分泌细胞中遗传力的最显著富集。
- 这表明遗传变异可能会影响肠内分泌细胞，从而对BMI产生影响。

3. **对照实验（未观察到牙齿龋齿遗传力富集）**：
- 作为对照，研究测试了GWAS SNPs在一个无关的性状（牙齿龋齿）中的遗传力富集情况，发现没有细胞类型特异性的富集。

这些结果将重要的疾病性状映射到肠道中的特定细胞类型上，揭示了疾病遗传力与细胞类型的关系。

### 图例:
Figure 6f

![](images/2024-05-15-16-27-35.png)

连锁-不平衡评分回归以确定 GWAS SNP 的富集细胞类型特异性标记峰。计算未调整系数 P 值连锁不平衡得分回归绘制在热图中。重要性用星号表示，由 Bonferroni- 确定修正系数P值<0.05。用于确定显着性的 P 值根据测试的细胞类别数量进行调整。

### 比喻讲解：

可以将这项研究想象成一场大型的侦探调查，目的是找出哪些肠道细胞类型是特定疾病的“罪犯”。这场调查使用了一种叫做连锁不平衡得分回归的方法，就像是使用一个高级的侦探工具来追踪遗传线索。

1. **T细胞和疾病的关系**：

- 想象T细胞是一个侦探，他们在调查克罗恩病和乳糜泻案件时，发现这些疾病在T细胞标记峰上的“指纹”非常明显。这就像在犯罪现场找到了明确的证据，指向T细胞在这些疾病中扮演了重要角色。

2. **BMI和肠内分泌细胞的关系**：

- 研究人员发现BMI（身体质量指数）的遗传变异在肠内分泌细胞中表现最为显著。这就像是发现肠内分泌细胞在管理身体体重的过程中，可能是幕后操纵者，影响着整个身体的体重平衡。

3. **对照实验中的发现**：

- 作为对照，研究人员也检查了牙齿龋齿的遗传变异，结果发现并没有任何肠道细胞类型与之显著相关。这就像是在一场完全无关的案件中，没有找到任何肠道细胞的“指纹”，证实了他们只在特定疾病中扮演重要角色。

通过这场侦探调查，研究人员成功地将一些重要的疾病特征与肠道中的特定细胞类型联系起来，就像是破解了一些复杂案件，找到了关键的“罪犯”。
...
这段文本描述了一项研究中的伦理合规和实验方法，具体涉及人类肠组织的获取、保存和处理步骤。以下是详细讲解：

1. **伦理合规**：
- 研究遵循所有相关的伦理规定，并获得了华盛顿大学和斯坦福大学机构审查委员会的批准。
- 人类肠组织来自已故器官捐献者，所有捐献者的近亲属都签署了书面知情同意书。

2. **器官保存和处理**：
- 器官按照外科手术规程进行保存，以准备肠道移植。
- 具体步骤包括：
1. 在循环停止前，将大口径导管插入肾下动脉。
2. 循环停止后，立即通过动脉导管用冰冷的HTK保存溶液冲洗腹腔脏器，并用冰包装腹腔，快速冷却腹部内脏。
3. 肠道在运输和解剖过程中保持低温，直到样本可以冷冻以进行长期保存。

3. **样本保存**：
- 从指定的肠道部位收集的组织样本通过液氮快速冷冻（snap freezing）保存，用于单核RNA测序（snRNA-seq）和单细胞ATAC测序（scATAC）。
- 样本也嵌入并冷冻在最佳切割温度化合物（OCT）中，用于CODEX（共定位解码实验）。

这些步骤确保了从获取到保存的全过程中，组织样本的质量和完整性，为后续的实验提供了可靠的材料基础。
...
该研究收集了来自四名捐赠者的成像数据，每名捐赠者的数据构成一个数据集。每个数据集包括两组组织，这些组织在同一个盖玻片上一起成像，每组组织包含四个组织：

1. **大肠**：
- **部位**：乙状结肠、降结肠、横结肠和升结肠。

2. **小肠**：
- **部位**：回肠、中段空肠、近段空肠和十二指肠。

这些组织被分别冷冻在OCT模具中，然后被切割并组装成四个组织的阵列，这些组织具有已知的方向性，以便通过一次切割块体获得每个组织的横截面。这些组织阵列是在冷冻切片机（cryostat）上构建的，并以7微米的宽度切片。
...
该段文本描述了将组织块在HuBMAP（Human BioMolecular Atlas Program）的组织注册中的操作过程，该操作使用了一个共同的坐标框架。以下是详细讲解：

1. **参考对象的创建**：
- 使用Visible Human Project的数据集，创建了男性和女性11个器官（包括小肠和结肠）的3D参考对象。

2. **组织块的注册**：
- 使用标准的外科解剖标志，将八个肠道部位的组织块注册到这些参考对象中。

3. **小肠段的解剖标志**：
- 小肠段的解剖标志如下：
1. 胰头右侧的降部十二指肠；
2. 在空肠内Treitz韧带之外5厘米处；
3. 在中肠内Treitz韧带之外200厘米处的空肠段；
4. 在末端回肠距离回盲瓣近端5厘米处。
- 每个部位收集5厘米的肠段，代表大约20克的组织。

4. **结肠段的解剖标志**：
- 结肠段的解剖标志如下：
1. 右结肠在回盲瓣与肝曲之间的中点；
2. 横结肠在肝曲和脾曲之间的中点；
3. 左结肠在脾曲与乙状结肠系膜出现处之间的中点；
4. 乙状结肠在乙状结肠与直肠乙状结肠交界处的中点，即结肠带停止处。

通过这些步骤，研究人员能够准确地定位和收集肠道不同部位的组织样本，并将其注册到3D参考对象中，从而为进一步的研究提供标准化的数据基础。
...
该研究描述了使用CODEX多重成像技术进行实验的过程和方法。主要内容如下：

1. **CODEX多重成像的执行**：
- CODEX多重成像按照先前描述的CODEX染色和成像协议进行操作。CODEX技术是一种用于高维细胞和组织分析的成像技术。

2. **抗体面板选择**：
- 选择的抗体面板包括用于识别肠上皮细胞和基质细胞亚型，以及先天性和适应性免疫系统细胞的靶标。详细的面板信息可以在补充表7中找到。

3. **抗体与寡核苷酸条形码的结合**：
- 每个抗体都结合了一个独特的寡核苷酸条形码。随后，组织样本被这些抗体-寡核苷酸结合物染色，并验证染色模式与已建立的免疫组织化学染色模式一致，这些阳性对照组织来自肠或扁桃体。

4. **形态学验证**：
- 使用苏木精和伊红（hematoxylin and eosin，H&E）染色验证标记染色的位置。H&E染色是一种常用的组织学染色方法，用于显示组织的总体形态结构。

5. **抗体-寡核苷酸结合物的测试**：
- 首先在低通量荧光检测中测试抗体-寡核苷酸结合物，并在此步骤中评估信噪比。然后，将所有抗体-寡核苷酸结合物一起在单次CODEX多循环中进行测试。

该过程确保了染色的准确性和信号的可靠性，从而能够精确地进行细胞和组织分析。
...
这段文本描述了使用CODEX（Cyclic Immunofluorescence）技术进行组织芯片染色和成像的过程。具体步骤如下：

1. **染色**：
- 使用经过验证的CODEX抗体面板对组织芯片进行染色。这些抗体是专门设计和验证过的，确保能够准确识别目标分子。

2. **成像过程**：
- 成像过程包括循环剥离、退火和成像。具体来说，使用与连接物上的寡核苷酸互补的荧光标记寡核苷酸进行循环剥离和退火，然后进行成像。
- 这个过程的目标是确保每个抗体-寡核苷酸连接物的荧光信号能够被清晰地捕捉。

3. **测试和优化**：
- 在验证抗体-寡核苷酸连接物面板之后，进行CODEX多重检测实验。在这个过程中，再次评估信噪比，并为每个连接物确定最佳稀释度、曝光时间和适当的成像周期（详见补充表7）。
- 这一步是为了确保成像过程中每个目标分子的信号强度足够高，同时背景噪音尽可能低，从而获得高质量的成像数据。

4. **最终成像**：
- 每个组织芯片进行CODEX多重成像。每次CODEX运行的元数据记录在补充表8中。
- 元数据包括每次成像的详细参数和结果，有助于后续数据分析和验证。

通过上述步骤，研究人员能够利用CODEX技术对组织芯片进行高精度、多重成像，捕捉到丰富的分子信息，进而用于各种生物医学研究。
...
这段文本详细介绍了研究人员如何处理和分析原始成像数据以进行单细胞分割和细胞类型识别的过程。主要步骤和方法如下：

1. **原始数据处理**：
- 使用CODEX Uploader进行图像拼接、漂移补偿、去卷积和循环连接。这些步骤确保了图像数据的一致性和高质量。

2. **数据分割**：
- 处理后的数据通过CODEX Segmenter或CellVisionSegmenter进行单细胞分割。
- CODEX Segmenter使用基于分水岭算法的单细胞分割算法。
- CellVisionSegmenter使用基于神经网络R-CNN的单细胞分割算法。
- 样本B001使用CODEX Segmenter进行分割，其他所有样本使用CellVisionSegmenter进行分割。

3. **CellVisionSegmenter的优势**：
- CellVisionSegmenter对稠密和稀疏的细胞组织都能很好地分割。
- 它是一个开源的、预训练的核分割和信号量化软件，基于Mask R-CNN架构。
- CellVisionSegmenter的训练基于研究小组手动注释的CODEX多重成像数据。

4. **参数调整**：
- 研究人员发现核掩膜的增长像素值为3时效果最佳，尽管如此，任何分割算法在某些情况下都不能完美分割，可能会捕捉到相邻细胞的一部分。

5. **分割的局限性和影响**：
- 核分割可能会限制量化信号，而全细胞分割可能更好地捕捉信号，但也存在细胞膜染色不一致的问题。
- 研究人员对此进行了深入分析，以确保细胞类型识别的准确性。

6. **数据规范化和聚类分析**：
- 使用不同的数据规范化技术和无监督聚类方法（如z-normalization、Vortex或Leiden-based clustering）对数据进行分析。
- 通过对数据进行过度聚类，并手动将结果细胞类型聚类叠加到图像上，可以显著提高细胞类型识别的准确性。

这段文本展示了研究人员通过精细的数据处理和分析流程，克服单细胞分割和识别中的挑战，从而在高精度的细胞类型识别中取得了成功。
...
这段文本描述了如何下载、使用和评估CODEX Uploader、Segmenter软件以及CellVisionSegmenter软件，以处理生物医学图像。具体步骤如下：

1. **软件下载**：
- **CODEX Uploader和Segmenter软件**：
- 可以从GitHub下载，网址为：https://github.com/nolanlab/CODEX
- **CellVisionSegmenter软件**：
- 可以从GitHub下载，网址分别为：https://github.com/bmyury/CellVisionSegmenter 或 https://github.com/michael-lee1/CellSeg

2. **图像上传和评估**：
- 在上传图像后，需要再次评估图像的特定信号。
- 任何产生不合适模式或信噪比低的标记将被排除在后续分析之外。

3. **图像可视化**：
- 上传的图像会在ImageJ中进行可视化，网址为：https://imagej.nih.gov/ij/

通过这些步骤，研究人员能够确保图像数据的质量和准确性，从而为后续的分析提供可靠的基础。
...
该文本详细描述了细胞类型鉴定的方法，主要针对001和B004细胞类型的识别过程。具体步骤如下：

1. **细胞选择**：
- 通过DRAQ5和Hoechst双阳性细胞的门控（gating）来选择有核细胞。

2. **蛋白标记的z标准化**：
- 使用用于聚类的蛋白标记物进行z标准化处理（z-normalization）。一些表型标记物未用于无监督聚类。

3. **移除高荧光标记的细胞**：
- 对于z值大于1且具有超过35个荧光标记的细胞，从数据中移除。

4. **数据聚类**：
- 使用X-shift（https://github.com/nolanlab/vortex）或基于Leiden的聚类方法（使用scanpy Python包，版本1.9.1）对数据进行过度聚类。

5. **处理步骤的基础**：
- 这些处理步骤基于对CODEX多重成像数据的归一化技术和无监督聚类算法的深入分析。此方法并非新技术，已有许多软件包将这些聚类算法集成到库中，如Squidpy。

6. **细胞类型的分配**：
- 根据平均聚类蛋白表达和图像中的位置，给聚类分配细胞类型。

7. **纯度不高的聚类处理**：
- 对于纯度不高的聚类，通过映射回原始荧光图像进行拆分或重新聚类。

这个过程确保了细胞类型鉴定的准确性，尤其是在处理肠道CODEX多重成像数据时。
...
在这段文本中，研究人员描述了他们如何利用STELLAR框架将CODEX细胞类型标签转移到其他供体上，以注释空间解析单细胞数据。以下是详细的讲解：

1. **STELLAR框架简介**：
- STELLAR是一种几何深度学习方法，利用空间和分子细胞信息，在不同数据集之间转移细胞类型标签。与其他方法（如SpaGCN和Spage2vec）不同，STELLAR不需要手动干预来分配细胞类型。
- SpaGCN和Spage2vec是无监督聚类方法，需要手动分配细胞簇，并可能需要多次手动调整聚类结果。

2. **STELLAR的优点**：
- 自动识别现有细胞类型并发现新细胞类型，无需手动干预。
- 因此，研究人员选择STELLAR作为首选方法。

3. **应用STELLAR的过程**：
- 研究人员使用供体B004的数据作为标记的训练数据集。该数据集通过聚类、合并、再聚类、子聚类等方法进行整理和注释，并根据平均标记表达将细胞分配到不同类型。
- 通过在CODEX图像中的细胞位置及相应的荧光图像和H&E染色来确认每个聚类的纯度和准确性。
- 使用供体B004、B005和B006的数据来训练STELLAR，并将注释转移到所有其他供体数据集上，这些数据集被视为未注释的测试数据集。

4. **质量确认**：
- 研究人员假设不同供体之间不会发现新的细胞类型，因此使用STELLAR识别现有的细胞类型。
- 所有数据集都进行了z标准化。
- 通过查看预测的细胞类型的平均标记表达谱，手动确认了STELLAR的细胞类型分配质量。发现蛋白质标记分布与专家手动注释的谱相匹配。

总的来说，这段文本介绍了研究人员如何利用STELLAR框架来有效地注释和转移细胞类型标签，减少了手动干预，并确保了高质量的注释结果。
...
在这项研究中，研究人员对细胞类型百分比进行了归一化处理，归一化到整体细胞类别，原因有以下三点：

1. **截取肠道样本的横截面**：
- 研究人员对每个肠道样本的横截面进行捕捉，但其中有少数样本（64个中的4个）并不是代表性的横截面。这意味着这些样本可能无法准确反映整体情况，因此需要进行归一化处理，以减少这些非代表性样本对结果的影响。

2. **使用固定的CODEX成像窗口**：
- 研究人员尝试捕捉完整的粘膜区域，因为粘膜区域包含了大多数通过抗体面板识别的细胞类型。这导致在不同样本中捕捉到的肌层外膜（muscularis externa）量有所不同。归一化处理有助于解决这种可变性，确保结果的可比性。

3. **理解细胞类型组成的变化**：
- 通过归一化到整体细胞类型，可以更好地理解不同细胞类型在三个不同区域中的组成变化。这有助于研究人员分析和比较不同区域中细胞类型的分布和比例。

综上所述，归一化处理有助于减少样本之间的差异，确保研究结果的准确性和一致性，并提供对细胞类型组成变化的深入理解。
...
该段文本描述了使用CODEX（Codetection by Indexing）的技术来分析同种细胞的密度。具体方法如下：

1. **同种细胞密度的分析**：
- 首先，计算每个图像区域中每个细胞到同种细胞的五个最近邻居的平均距离。
- 将该平均距离除以同种细胞的最弥散距离。

2. **最弥散距离的计算**：
- 通过将特定类型的细胞总数除以该区域的总面积来计算同种细胞的最弥散距离。

3. **密度数值的解释**：
- 数值越接近1，表示细胞的密度越低。
- 数值越接近0，表示细胞的密度越高。

这种方法通过计算每个细胞与其最近邻居的距离，并将其标准化，来评估细胞密度的分布情况。通过这种方式，研究人员能够量化和比较不同区域内同种细胞的密度差异。
...
这段文本描述了一种用于分析组织中细胞类型分布的邻域分析方法。具体步骤如下：

1. **选择细胞窗口**：
- 将整个组织的细胞类型图划分为多个窗口，每个窗口以一个细胞为中心。
...
4. **确定总体结构**：
- 根据聚类的平均组成确定总体结构（参见扩展数据图2a）。

在这个过程中，选择合适的窗口大小是关键。窗口大小影响到分析的细节程度：
- **较小的窗口**：识别局部或微观结构。
- **较大的窗口**：识别需要较大窗口的相似组成结构。

在本研究中，研究人员选择了窗口大小的标准是选取每个细胞周围的十个最近邻居。这一数值在识别代表细胞直接微环境或局部邻居的保守组成方面效果很好。

研究人员这样选择是为了在邻域层次上观察微观结构，并进一步了解这些微观结构如何协同工作并形成肠道的宏观结构。总体而言，结构的大小并不直接与窗口大小相关，而是与保守细胞类型在给定结构中的划分程度相关。

最后，研究人员将邻域过度聚类为30个簇。然后将这些簇映射回组织，并评估细胞类型富集情况以确定总体结构，最终合并为20个独特结构。
...
该研究的目的是确定小肠和结肠之间在邻域组成（细胞类型）上的保守性。具体过程如下：

1. **邻域富集评分的计算**：
- 研究人员分别计算了小肠和结肠样本中每个供体的邻域富集评分。
- 邻域富集评分的计算方法是：邻域簇内的平均细胞类型百分比除以组织中所有细胞的平均细胞类型百分比。

2. **比较小肠和结肠的富集评分**：
- 将每种细胞类型在结肠中的富集评分减去其在小肠中的富集评分。
- 根据这些差异值，研究人员生成了热图，按照邻域和细胞类型在保守性上的最大绝对差异排序。

3. **结果解释**：
- 对于某个特定邻域，如果某种细胞类型的富集评分在小肠和结肠之间存在显著差异，则说明该细胞类型在该邻域结构中在小肠和结肠之间的组成保守性较低，即其组成在小肠和结肠之间不一致。

这项研究通过分析小肠和结肠中细胞类型在不同邻域的富集情况，揭示了在这些肠道部分中细胞类型组成保守性的重要差异。
...
该研究通过一种类似于确定多细胞邻域的方法来确定社区，但在方法上有一些小的差异。具体过程如下：

1. **选择邻域细胞**：
- 在邻域组织图中，选择了窗口大小为100的最近邻居细胞。这意味着每次选择的细胞群体由其周围最近的100个邻居细胞组成。

2. **覆盖整个组织**：
- 这些窗口覆盖了整个组织，确保了数据的全面性。

3. **聚类分析**：
- 通过k-means聚类算法对这些细胞群体进行聚类分析。为了避免过度聚类，采用了总共20个簇（clusters）的设置。

4. **映射和评估**：
- 将这些聚类结果映射回组织，并评估每个聚类的邻域组成和富集情况，以确定总体的社区类型。

同样的方法也应用于确定社区，但窗口大小改为300个最近邻居。这种方法确保了对组织中更大范围内细胞相互作用和组成的准确识别。

通过这种方法，研究人员能够详细地分析组织中不同区域的细胞组成和相互作用，进而确定不同的社区类型。这对于理解组织结构和功能具有重要意义。
...
这段文本描述了一个层次结构模型中各级别之间的连接和表示方式。具体内容如下：

1. **层次级别之间的连接**：
- 每个层次级别与下一个层次级别之间的连接有两种方式：
- 一种是通过贡献最大值的特征（feature）来连接。
- 另一种是通过占下一个层次级别至少15%的特征来连接。

2. **特征的表示**：
- 在每个层次级别中，每个特征所占的百分比通过形状的大小来表示。
- 形状和颜色的组合对应于特定的层次和特征。

3. **连接线的表示**：
- 连接线的大小表示一个特征对下一个层次级别中特征的贡献量。

总的来说，这段描述解释了在一个层次模型中，如何通过特征之间的贡献关系以及它们在每个层次中的比例来表示和连接各个层次。
...
该文本描述了空间上下文图的创建方法及其分析过程，具体内容如下：

1. **空间上下文分析的概述**：
- 研究人员利用了一种空间上下文分析方法来研究邻里（neighbourhood）和社区（community）之间的关联。
- 这种方法与识别多细胞邻里的方法有一些相似之处，但也存在一些关键差异。

2. **计算邻里和社区窗口**：
- 邻里窗口：计算了每个邻里窗口中最近的100个邻居。
- 社区窗口：计算了每个社区窗口中最近的300个邻居。

3. **邻里和社区空间上下文图的不同**：
- 邻里空间上下文图：仅包含粘膜组织单元中的细胞。
- 社区空间上下文图：包含所有组织中的所有细胞。

4. **组合的选择和表示**：
- 一旦窗口计算完成（即窗口内每种细胞类型的数量），选择在该窗口中代表超过85%的组合。
- 这种组合反映了窗口中邻里或社区的显著关联，被称为空间上下文。
- 这些组合被计数，并通过在方形邻里组合下方的黑色圆圈的大小来表示其大小。
- 仅绘制了频率超过0.1%的组合用于可视化。

5. **组合的连接和图形表示**：
- 每个组合连接到包含它的每个其他组合，在图的子层中用另一种组合连接。例如：
- 如果分泌上皮社区（Secretory Epithelial community）占一个窗口的85%，则这是一个单独的组合（用紫色三角形表示）。
- 如果它与适应性免疫富集社区（Adaptive-Immune-Enriched community）一起存在，则它们会连接成一个组合（紫色三角形和橙色三角形）。
- 类似地，如果它与浆细胞富集社区（Plasma-Cell-Enriched community）一起存在，则它们会连接成另一个组合（紫色三角形和黄色三角形）。
- 单个组合通过黑色边连接到这些组合（节点）在空间上下文图中。
- 类似地，分泌上皮和适应性免疫富集社区的组合（紫色三角形和橙色三角形）可能进一步与其他社区组合（例如，滤泡社区，蓝色三角形）衍生出更多组合（例如，紫色、橙色和蓝色三角形的组合）。
...
这段文本描述了如何使用一种先前开发的方法来识别显著相关的细胞邻域-邻域基序（cellular neighbourhood-neighbourhood motifs）。以下是详细讲解：

1. **方法概述**：
- 研究使用了一种先前开发的方法来识别显著相关的细胞邻域-邻域基序。
- 基序识别利用的是组织中同一群体的多个细胞共定位的分割区域，而不是单个细胞（如扩展数据图5a所示）。

2. **组织网络图的构建**：
- 组织网络图代表了群体实例之间共享的边。
- 为每个治疗组创建组织图时，我们取每个独特成像区域的组织图的并集。

3. **创建空集（null-set）**：
- 空集是通过对细胞邻域或群体分配进行一系列有效换位（transpositions）创建的。
- 通过对邻域或群体分配进行置换并固定顶点数，创建了最大熵（maximum entropy）空分布。

4. **显著性链的识别**：
- 仅考虑至少有五个实例的两个链。
- 为了识别显著的链，对P值进行了Bonferroni校正，校正方法是将P值乘以每个比较组（小肠和CL）中进行的测试次数的两倍。

总结起来，研究通过构建组织网络图并进行邻域或群体分配的置换，生成空分布，然后使用Bonferroni校正来识别显著的细胞邻域-邻域基序。这种方法有助于揭示组织中细胞群体间的显著关联。
...
这段文本描述了一种名为OmniATAC的实验协议，用于分离细胞核。具体步骤如下：

1. **初始处理**：
- 使用40-60毫克闪冻组织，在1毫升HB（裂解）缓冲液中轻轻研磨并解冻5分钟。
- HB缓冲液成分包括：1.0341× HB稳定溶液，1M DTT，500mM精胺，150mM腐胺，10% NP40，完全蛋白酶抑制剂和Ribolock。

2. **研磨和过滤**：
- 使用A杵捣碎10次，B杵捣碎20次，直到没有阻力。
- 通过40µm细胞过滤器过滤样品，得到的匀浆液转移到预冷的2ml LoBind管中。

3. **离心和重悬**：
- 在4°C固定角度离心机中以350 rcf离心5分钟以沉淀细胞核。
- 离心后去除所有上清液，仅保留50µl。加入350µl HB，使总体积达到400µl，使用宽口移液器轻轻重悬细胞核。

4. **碘克沙醇密度梯度分离**：
- 加入一体积50%碘克沙醇（60% OptiPrep，稀释缓冲液（2M KCl，1M MgCl2，0.75M Tricine-KOH，pH 7.8），水），轻轻研磨。
- 小心在25%混合液下层加入600µl 30%碘克沙醇，再在30%混合液下层加入600µl 40%碘克沙醇。

5. **密度梯度离心**：
- 在4°C摆动桶离心机中以3000 rcf离心20分钟，形成可见的细胞核带。
- 吸出上清液，直到距离细胞核带200-300µl。
- 收集200µl的细胞核带，转移到新的1.5ml管中。

6. **细胞核重悬和计数**：
- 加入等体积（200µl）重悬缓冲液（1× PBS，1% BSA，0.2 U/µl Ribolock）。
- 使用Countess II FL自动细胞计数器（Thermo Fisher Scientific）确定细胞核浓度。
...
该段文本描述了一种用于研究细胞核基因组的技术，具体细节如下：

1. **snATAC-seq技术**：
- snATAC-seq（单核ATAC测序）是一种用于分析单个细胞核染色质开放状态的技术。在这个实验中，每个样本目标是9000个细胞核。

2. **使用的试剂盒和芯片**：
- 使用了10x Genomics公司的Chromium Next GEM Single Cell ATAC Library & Gel Bead Kit v1.1（产品编号：1000175）和Chromium Next GEM Chip H（产品编号：1000161）或者Chromium Single Cell ATAC Library & Gel Bead Kit（产品编号：1000110）。

3. **文库测序**：
- 文库在Illumina NovaSeq 6000系统上进行测序。
- 加载浓度为1.4 pM，读长配置为50 × 8 × 16 × 49 bp（碱基对）。
- 每个细胞核目标平均读数为25,000次。

这段描述详细说明了snATAC-seq技术的实验流程、使用的试剂和设备以及测序的具体参数。
...
该研究使用单核RNA测序（snRNA-seq）技术，对每个样本中约9,000个细胞核进行分析。具体实验步骤如下：

1. **实验试剂与设备**：
- 使用了Chromium Next GEM Single Cell 3’ Reagent Kits v3.1（10x Genomics, 1000121）和Chromium Next GEM Chip G Single Cell Kit（10x Genomics, 1000120）。
- 这些试剂盒和芯片是用于单细胞和单核RNA测序的先进工具，能够高效地捕获和分析单个细胞或细胞核的RNA表达情况。

2. **文库构建与测序**：
- 构建了测序文库（libraries），并将这些文库在Illumina NovaSeq 6000系统上进行测序。
- 测序配置为：read 1 = 28 bp（碱基对），i7 index = 8 bp，i5 index = 0 bp，read 2 = 91 bp。这些参数定义了测序读段的长度和索引标签的长度，用于区分不同的样本或细胞。

3. **目标数据量**：
- 每个细胞核的目标读数为平均20,000次。这意味着每个细胞核的RNA序列被测序大约20,000次，以确保获得足够的深度和准确性来分析基因表达情况。

通过这种高通量的单核RNA测序技术，研究人员能够详细分析每个细胞核的基因表达谱，从而深入了解不同细胞类型的功能和状态。
...
这段文本描述了使用Chromium Next GEM单细胞多组学（Multiome）技术进行的实验细节。目标是每个样本中处理9,000个细胞核。以下是实验的具体步骤和配置：

1. **实验技术**：
- 使用了10x Genomics公司提供的Chromium Next GEM Single Cell Multiome ATAC + Gene Expression试剂盒（产品编号：1000283）。
- 该技术能够同时测量单个细胞的ATAC-seq（染色质可及性测序）和基因表达（RNA-seq）数据。

2. **ATAC-seq文库构建和测序**：
- 读数1（read 1）长度为50个碱基对（bp）。
- i7索引长度为8个碱基对。
- i5索引长度为24个碱基对。
- 读数2（read 2）长度为49个碱基对。

3. **RNA-seq文库构建和测序**：
- 读数1（read 1）长度为28个碱基对。
- i7索引长度为10个碱基对。
- i5索引长度为10个碱基对。
- 读数2（read 2）长度为90个碱基对。

4. **测序平台**：
- 使用Illumina NovaSeq 6000系统对ATAC-seq和RNA-seq文库分别进行测序。

总结来说，这项实验使用先进的多组学技术，能够同时获取单细胞水平上的染色质可及性和基因表达数据，为深入理解细胞功能和状态提供了重要工具。
...
这段文本介绍了单细胞ATAC-seq (scATAC-seq) 数据、单核RNA-seq (snRNA-seq) 数据以及多组学数据(multiome data)的初始处理步骤，具体使用了不同的Cell Ranger流水线。以下是详细解释：

1. **scATAC-seq数据的初始处理**：
- 使用了Cell Ranger ATAC流水线。
- 首先运行`cellranger-atac mkfastq`来将bcl文件去复用（demultiplex），将测序数据转换成fastq文件。
- 然后运行`cellranger-atac count`来生成scATAC片段文件（fragments files）。

2. **snRNA-seq数据的初始处理**：
- 使用了Cell Ranger流水线。
- 首先运行`cellranger mkfastq`来将bcl文件去复用，转换成fastq文件。
- 然后运行`cellranger count`进行计数，因为是核RNA测序（nuclear RNA sequencing），所以数据对齐到了pre-mRNA参考基因组（pre-mRNA reference）。

3. **多组学数据的初始处理**：
- 使用了Cell Ranger ARC流水线。
- 处理步骤包括数据对齐（alignment）、生成片段文件（fragments files）和表达矩阵（expression matrices）。

每个流程都详细描述了使用的具体命令和步骤，确保了不同类型数据的初始处理能够有效进行。
...
这段文本描述了使用CODEX数据计算和比较小肠和结肠中所有细胞类型对之间的共定位商（CLQ，Colocalization Quotient）。具体方法如下：

1. **共定位商（CLQ）的计算公式**：
- CLQ的计算公式为：\(\mathrm{CLQ}_{\mathrm{A} \rightarrow \mathrm{B}}=\frac{C_{\mathrm{A} \rightarrow \mathrm{B}} / N_{\mathrm{A}}}{N_{\mathrm{B}} /(N-1)}\)
- 其中，\(C_{\mathrm{A} \rightarrow \mathrm{B}}\) 表示细胞类型A在细胞类型B的最近邻居中出现的次数。
- \(N\) 是细胞总数。
- \(N_{\mathrm{A}}\) 和 \(N_{\mathrm{B}}\) 分别表示细胞类型A和细胞类型B的数量。

2. **计算目的**：
- 研究的目的是比较小肠和结肠中不同细胞类型对之间的CLQ，以了解这些细胞类型在不同部位的分布差异。

3. **统计分析方法**：
- 使用经过多重假设检验校正的Student's t检验来识别小肠和结肠之间具有显著差异的CLQ。

总的来说，这项研究通过计算CLQ，分析了小肠和结肠中不同细胞类型的分布和共定位情况，并使用统计方法验证了这些差异的显著性。
...
该段文本描述了研究如何获得并分析配体和受体对的过程，具体步骤如下：

1. **数据来源**：
- 研究使用了FANTOM5数据库及其他12对文献支持并实验验证的配体和受体对，来获得最终的已验证配体受体对的列表。

2. **差异表达分析**：
- 研究采用非参数的Wilcoxon秩和检验来识别在小肠与结肠中差异表达的配体和受体。 
- 设定调整后的P值阈值为0.05，以确保差异表达的显著性。

这个过程的核心在于从多个可靠的来源获取配体和受体对，并使用统计方法来分析这些配体和受体在不同肠道部位的表达差异，从而揭示潜在的生物学差异和机制。
...
这段文本描述了使用ArchR软件包在R语言环境中处理snATAC-seq数据的详细步骤，具体如下：

1. **数据加载和质量控制**：
- 将snATAC-seq片段文件加载到R中，使用ArchR软件包中的createArrowFiles函数。
- 计算每个细胞的质量控制指标，对于TSS（转录起始位点）富集度小于5的细胞进行剔除。
- 根据每个样本的独特片段数目进行细胞过滤，定义了每个样本的独特片段截断值，范围从1,000到10,000片段/细胞，最常见的截断值是3,000片段/细胞。这个样本特定的截断值考虑到了不同样本间测序深度的差异。

2. **双细胞检测和过滤**：
- 质量控制和过滤后，使用ArchR函数addDoubletScores计算所有multiome细胞和所有非multiome snATAC细胞的双细胞（doublet）得分，参数设定为k=10，knnMethod=“UMAP”，LSIMethod=1。
- 创建一个ArchR项目，并使用filterDoublets函数以1.2的过滤比例过滤双细胞。
- 一小部分snATAC样本未能分离成预期的细胞类型集群，这些样本被从后续分析中移除。

3. **降维分析和聚类**：
- 对于非multiome的scATAC细胞，使用addIterativeLSI生成IterativeLSI降维分析，参数设定为iterations=3，sampleCellsPre=25000，dimsToUse=1:25和varFeatures=15000。
- 使用addClusters函数进行聚类，参数设定为resolution=1.5，nOutlier=20，seed=1，sampleCells=40000和maxClusters=40。
- 根据细胞在已知的免疫、基质或上皮标记基因上的高基因活性评分，将结果集群分成不同组别。这些基因活性评分反映了基因体内及其周围的可及性。
...
这段文本详细描述了在单核RNA测序（snRNA-seq）数据分析过程中使用的一些数据预处理步骤，具体如下：

1. **数据读取和过滤**：
- 使用Cell Ranger软件生成的原始特征条形码矩阵（raw_feature_bc_matrix）通过Seurat包中的`Read10X`函数读取到R中。
- 进行了数据过滤，去除了以下特征的核：
- 每个核包含少于400个独特基因。
- 每个核包含大于或等于10,000个基因。
- 每个核包含大于或等于20,000个计数。
- 每个核包含大于或等于5%的线粒体RNA。
- 为了限制环境RNA的影响，过滤掉那些没有至少是所有液滴中计数中位数三倍计数的核。这是因为大多数液滴是空的，这样可以限制数据集中液滴中的RNA来自环境RNA的比例。

2. **双重细胞（doublets）的识别和去除**：
- 对每个非多组学的snRNA样本使用DoubletFinder进行双重细胞的识别，使用了主成分分析的前1-20个主成分。
- `nExp`设置为0.076 × nCells² / 10,000，`pN`设置为0.25，而`pK`通过`paramSweep_v3`函数确定。
- 在下游分析之前，去除了被分类为双重细胞的细胞。

3. **环境RNA的校正**：
- 使用DecontX软件对多组学和非多组学细胞的snRNA数据进行了可能的环境RNA校正。



这段文本描述了如何处理和分析单细胞RNA测序数据，具体步骤如下：

1. **合并数据**：
- 将所有样本中的剩余细胞合并成一个Seurat对象。

2. **数据处理**：
- 使用Seurat的标准流程进行数据处理。
- 首先，使用`NormalizeData`函数进行数据标准化，方法为`LogNormalize`，比例因子为10,000。
- 然后，使用`findVariableFeatures`函数，通过`vst`方法识别2,000个变异特征。

3. **数据缩放和主成分分析**：
- 对所有基因运行`ScaleData`进行数据缩放。
- 使用`RunPCA`函数计算主成分。

4. **消除批次效应**：
- 为了在聚类过程中消除不同供体之间的批次效应，使用`RunHarmony`函数，将数据按供体进行分组。

5. **降维和聚类**：
- 使用`RunUMAP`函数从harmony降维结果中生成UMAP降维图。
- 使用`FindNeighbors`函数，参数`reduction`设为“harmony”，`dims`设为1到20，来找到邻居。
- 使用`FindClusters`函数，分辨率设为1，对细胞进行聚类。

6. **标记基因表达和分类**：
- 最后，通过分析标记基因在聚类结果中的表达情况，将聚类标记为上皮细胞、基质细胞或免疫细胞，以便进行后续分析。
...
这段文本详细描述了单细胞ATAC测序（snATAC）和单细胞RNA测序（snRNA）数据的注释过程及其分析方法。具体内容如下：

1. **数据分组**：
- snATAC和snRNA数据被注释为以下组别：十二指肠上皮细胞、空肠上皮细胞、回肠上皮细胞、结肠上皮细胞、基质细胞和免疫细胞。

2. **数据进一步分项目**：
- snATAC数据进一步分为多组学（multiome）和非多组学（nonmultiome）细胞项目。对于多组学细胞，使用RNA注释；对于仅snATAC的细胞，则单独注释。

3. **ATAC数据处理**：
- 每个组别中的细胞被分配到一个新的ArchR项目中。
- 运行`addIterativeLSI`对每个组别进行处理。
- 使用LSI维度作为输入运行`addHarmony`。
- 计算和谐维度后，使用`addClusters`对细胞进行聚类，并基于和谐坐标使用`addUMAP`计算UMAP。

4. **细胞类型注释**：
- 通过检查已知标记基因的基因活性得分对聚类进行注释。
- 标记基因用于初步注释细胞类型，包括：
- BEST4+肠细胞（BEST4, OTOP2）
- 杯状细胞（MUC2, TFF1, SYTL2）
- 未成熟杯状细胞（KLK1, RETNLB, CLCA1）
- 干细胞（RGMB, SMOC2, LGR5, ASCL2）
- 突细胞（SH2D6, TRPM5, BMX, LRMP, HCK）
- 肠内分泌细胞（SCGN, FEV, CHGA, PYY, GCG）
- 循环过渡放大细胞（TICRR, CDC25C）
- 帕内特细胞（LYZ, DEFA5）

这整个过程展示了如何通过一系列计算步骤和基因标记来精确注释和分类不同类型的肠道细胞，从而帮助更好地理解肠道生物学和相关疾病的机制。

*****



该研究对免疫系统中的细胞进行了详细的分类和分析，并识别了不同类型的免疫细胞和其他相关细胞类型。具体内容如下：

1. **免疫细胞分类**：
- **CD4+ 和 CD8+ T细胞**：标记基因包括CD2、CD3E、IL7R、CD4、CD8。
- **B细胞**：标记基因包括PAX5、MS4A1、CD19。
- **浆细胞**：标记基因包括IGLL5、AMPD1。
- **自然杀伤细胞（NK细胞）**：标记基因为SH2D1B。
- **巨噬细胞/单核细胞**：标记基因为CD14。
- **肥大细胞**：标记基因包括HDC、GATA2、TPSAB1。

1. **免疫细胞群的聚类分析**：
- 研究发现来自一位捐赠者的自然杀伤细胞和T细胞与其他T细胞单独聚类。这位捐赠者年仅24岁，比其他研究对象都要年轻（详见补充表1），这可能解释了这种聚类差异。

1. **平滑肌/肌成纤维细胞**：
- 这些细胞群显示出高表达ACTA2、MTH11和TAGLN基因。

1. **绒毛成纤维细胞**：
- 绒毛成纤维细胞显示出高表达WNT5B基因。
- 部分隐窝成纤维细胞显示出高表达RSPO3基因。

通过这些基因表达的分析和细胞群的分类，研究能够更好地理解免疫系统中的不同细胞类型及其在免疫反应中的角色和功能。


*****

该文本描述了对snRNA细胞进行数据处理和聚类分析的步骤，具体操作如下：

1. **将细胞分成六个Seurat对象**：
- 这些对象来自免疫和基质（stromal）区域的细胞，以及四个上皮细胞组。

2. **免疫和基质区域的细胞处理流程**：
- 对这些细胞进行了初步聚类分析，使用的步骤包括：NormalizeData（数据标准化）、ScaleData（数据缩放）、RunHarmony（去批效应）、FindNeighbors（寻找邻居）和FindClusters（聚类）。

3. **四个上皮细胞组的数据整合**：
- 使用不同的方法处理上皮细胞的数据。具体步骤如下：
- 首先，对每个样本中的上皮细胞运行SCTransform，设置assay为“decontXcounts”，方法为“glmGamPoi”，并调整“percent.mt”（线粒体基因百分比）和“percent.ribo”（核糖体基因百分比）两个变量。
- 接下来，使用Seurat函数SelectIntegrationFeatures选择3000个整合特征。
- 然后，运行PrepSCTIntegration和FindIntegrationAnchors，通过参考基础的整合方法（dims为1-30）找到整合锚点。
- 最后，运行IntegrateData进行数据整合。

4. **聚类分析**：
- 对整合后的数据进行RunPCA（主成分分析），然后运行FindNeighbors和FindClusters来对整合后的数据进行聚类。

总结来说，这个过程详细描述了如何对不同类型的细胞进行初步处理、数据整合和聚类分析，以便更好地理解它们的特性和相互关系。
...
在这段研究中，作者详细描述了如何识别和移除可能的双细胞（doublets），并对不同类型的肠细胞进行聚类和注释。具体步骤如下：

1. **去除双细胞**：
- 在模拟双细胞的基础上（如上所述），作者还通过下游聚类和注释过程识别出表达多个谱系标记的簇（例如，基质细胞和免疫细胞）。例如，一些最初与免疫细胞聚类的细胞表达了比预期更高水平的基质基因。

2. **处理表达多个谱系标记的细胞**：
- 作者首先对所有最初分类为免疫细胞的细胞进行聚类，并为每个聚类识别标记基因。
- 接下来，将这些标记基因与先前发布的结肠标记基因列表进行比较，以确定可能不包含单个免疫细胞的簇。
- 然后，将这些细胞移到基质或上皮细胞区间，并与最初分类为上皮或基质细胞的所有细胞进行聚类。
- 如果这些细胞与基质细胞相比表现出较高的免疫标记基因表达，则推断它们最有可能是免疫/基质双细胞，并在下游分析前移除这些细胞簇。

3. **上皮细胞的初步注释**：
- 在初步注释上皮细胞后，作者根据SCTransform方法将所有样本中的snRNA数据中的肠内分泌细胞进行整合。
- 对所有样本中的所有上皮细胞运行SCTransform并使用基于参考的整合方法对所有上皮细胞进行整合。

4. **肠内分泌细胞的处理**：
- 在整合所有细胞后，作者只选择肠内分泌细胞，并使用RunPCA计算主成分，用FindNeighbors和FindClusters进行聚类。
- 根据标记基因的表达注释已知的肠内分泌细胞亚型（图4和扩展数据图6h）。

5. **MUC6+、MUC5B+和外分泌细胞的注释**：
- 作者选择整合的十二指肠数据，使用RunPCA计算主成分，用FindNeighbors和FindClusters进行聚类。
- 根据CELA3B和CPB1的表达注释外分泌细胞，根据MUC5B和MUC6的表达注释MUC5B+和MUC6+细胞（图4h）。

6. **不同肠道区域间细胞类型丰度的比较**：
- 使用scCoda和Milo软件包进行不同肠道区域间细胞类型丰度的比较。
...
本文描述了使用MACS2和ArchR工具进行峰值调用（peak calling）的具体方法，特别是在不同类型的肠道细胞中定义峰值集。主要内容如下：

1. **峰值调用的工具和方法**：
- 使用MACS2进行峰值调用，使用ArchR进行数据处理和分析。

2. **细胞类型的划分**：
- 峰值集（peak sets）分别独立地为上皮细胞、基质细胞和免疫细胞定义。

3. **上皮细胞峰值集的生成**：
- 为了生成一个能捕获细胞类型特异性和位置特异性的上皮细胞联合峰值集，研究人员将细胞分成若干组。
- 每组生成伪体积重复样本（pseudobulk replicates），并在这些伪体积重复样本上调用峰值。
- 对每组的伪体积重复样本调用的峰值生成一个可重复的峰值集。
- 最后，使用ArchR中的方法，迭代地将各组的峰值集合并为一个联合峰值集。

4. **上皮细胞峰值集的具体分组**：
- 上皮细胞分组包括每种上皮细胞类型的细胞，将所有肠内分泌亚型合并。
- MUC5B+、MUC6+、外分泌和未知细胞被合并。
- 小肠的绒毛细胞（tuft cells）由于数量较少，被合并为一个组。

5. **免疫和基质细胞峰值集的生成**：
- 免疫细胞和基质细胞的峰值集按细胞类型划分，但不按位置划分，因为这些组中的细胞数量较少。

6. **综合峰值集的定义**：
- 最后，为免疫、基质和上皮细胞的所有细胞类型定义了一个额外的峰值集，用于确定连锁不平衡得分回归（linkage-disequilibrium score regression）的标记峰值。
...
这段文本介绍了如何将单细胞RNA测序（snRNA）数据分配到非多组学单细胞ATAC测序（snATAC）样本中的步骤。具体过程如下：

1. **数据来源**：
- 研究涉及到肠道的四个主要区域：十二指肠、空肠、回肠和结肠。分别对这些区域的snRNA和snATAC数据进行了处理。

2. **数据整合工具**：
- 使用了ArchR软件包中的`addGeneIntegrationMatrix`函数进行数据整合。ArchR是一个用于单细胞ATAC测序数据分析的工具包。
   
3. **参数设置**：
- `addGeneIntegrationMatrix`函数的参数设置为：`reducedDims = “Harmony”`和`useMatrix = “GeneScoreMatrix”`。
- `reducedDims = “Harmony”`：使用Harmony算法进行数据的降维处理。Harmony是一种用于多种数据类型整合的算法，可以有效地消除批次效应。
- `useMatrix = “GeneScoreMatrix”`：指定使用基因评分矩阵（GeneScoreMatrix）来进行整合。这种矩阵包含了基因在各个细胞中的表达得分。

通过这些步骤，研究人员能够将snRNA数据准确分配到非多组学的snATAC样本中，从而在单细胞水平上进行更精细的基因组学和转录组学的整合分析。
...
该研究通过以下方法识别了转录因子（TF）调控因子，并在图5b中进行了展示。

1. **转录因子调控因子的识别**：
- 使用ArchR手册中的方法识别每个区域的转录因子调控因子，相关性阈值设定为0.5。
- 满足任意四个肠道主要区域调控因子标准的转录因子会被绘制在图5b中。

2. **数据集的处理**：
- 这一过程分别在多组学数据集（multiome datasets）和整合单组学数据集（integrated singleome datasets）中进行。
- 对于每个肠道区域中细胞数量较少的细胞类型（例如L细胞），将其合并为一个单独的组，不考虑其起源位置。最终的细胞类型分组显示在图5b的x轴上。

3. **免疫和基质细胞中的调控因子识别**：
- 分别在免疫和基质细胞中识别调控因子，最终的细胞类型分组显示在Extended Data图10g和10h的x轴上。

4. **转录因子足迹的计算**：
- 使用ArchR函数getFootprints和plotFootprints计算转录因子足迹。

通过这种方法，研究人员成功识别并展示了不同肠道区域中转录因子调控因子的分布和作用。
...
这段文本描述了研究人员如何利用ArchR包分析结肠各主要部分（十二指肠、空肠、回肠和结肠）的吸收性分化轨迹，详细过程如下：

1. **分化轨迹的推断**：
- 使用ArchR函数`addTrajectory`，将轨迹设置为从干细胞到TA2到TA1到未成熟肠细胞再到成熟肠细胞的和谐聚类（harmony clusters），并将`reducedDims`设置为和谐维度。

2. **轨迹上可变峰值的识别**：
- 首先使用`getTrajectory`生成一个包含轨迹上所有峰值可访问性的矩阵，参数`useMatrix`设为“PeakMatrix”，`log2Norm`设为TRUE。
- 使用`plotTrajectoryHeatmap`函数，参数包括`varCutOff`设为0.9，`returnMatrix`设为TRUE，`scaleRows`设为FALSE，`maxFeatures`设为100000，识别在任何一个区域中方差大于0.9的峰值。

3. **矩阵处理**：
- 将`getTrajectory`返回的四个矩阵连接成一个单一矩阵，并筛选出至少在一个区域中方差大于0.9且幅度差异至少为0.2的峰值。
- 使用ArchR函数`.rowZscore`计算结果矩阵的行z分数，并用`kmeans`函数进行k均值聚类，聚类数设为7，最大迭代次数为500。

4. **特征峰的富集分析**：
- 使用`peakAnnoEnrichment`计算标记峰中motif的超几何富集，结果的P值绘制在扩展数据图10k中。

5. **沿轨迹的可变基因识别**：
- 使用类似的方法识别沿轨迹的可变基因。对于多组学数据，使用`GeneExpressionMatrix`，对于单独的snATAC和snRNA数据，使用`GeneIntegrationMatrix`，替代`PeakMatrix`运行`getTrajectory`和`plotTrajectoryHeatmap`。
- 基因表达矩阵的`log2Norm`设为TRUE，仅包含幅度差异至少为0.5的基因。行z分数计算后使用`kmeans`聚类。

6. **基因集的KEGG路径富集**：
- 使用limma函数`kegga`确定这些基因集的KEGG路径富集，结果的未经调整的P值绘制在扩展数据图10l中。

7. **基因表达与伪时间的绘图**：
- 使用ArchR函数`plotTrajectory`生成基因表达与伪时间的关系图，使用`addImputeWeights`添加的imputeWeights。

8. **TFs与RNA表达的相关性**：
- 使用`correlateTrajectories`识别与motif活动和RNA表达相关的TFs，并绘制在图6d中。使用`addPeak2GeneLinks`识别与基因表达相关的峰值。

9. **具体基因的峰值相关性分析**：
- 图6e中，确定与ETV6表达相关的峰值，相关性至少为0.4；对于TMPRSS15，使用0.55的相关性阈值，显示最相关的峰值。

10. **数据集处理**：
- 以上过程分别对多组学数据集和集成单细胞组学数据集进行。

这段研究展示了通过复杂的计算分析和生物信息学方法，揭示结肠各部分吸收性分化的遗传调控机制。
...
这段文本详细描述了使用连锁不平衡得分回归（linkage-disequilibrium score regression，简称LDSC）方法进行细胞类型特异性分析的步骤。以下是逐步讲解：

1. **连锁不平衡得分回归的目的**：
- LDSC的目的是区分遗传力和混杂因素，例如人口分层和隐性亲缘关系。

2. **计算粗细胞类型的标记峰**：
- 在我们的数据集中计算粗细胞类型的标记峰。
- 首先，将细胞类型注释添加到包含所有细胞的完整ArchR项目中。
- 然后，通过运行`addGroupCoverages`（参数`groupBy = "CellType"`）和`addReproduciblePeakSet`以及`addPeakMatrix`来定义该对象的峰集。
...
- 将所有非干吸收性上皮细胞合并为一组。
- 将循环TA细胞合并为一组。
- 将所有肠内分泌细胞（除了EnteroendocrineUn）合并为一组。
- 将淋巴内皮细胞和内皮细胞合并为一组。

4. **确定标记峰**：
- 使用`getMarkerFeatures`为合并后的细胞组确定标记峰。
- 使用`getMarkers`（参数`cutOff = "FDR <= 0.1 & Log2FC >= 0.5"`）选择10个标记峰。
- 未包含细胞数量很少的细胞类型（如T细胞、未知细胞、外分泌细胞、MUC5B+分泌专业细胞、DC、ILC、脂肪细胞、神经元、EnteroendocrineUn）的标记峰。
- 将所得峰从hg38转换到hg19。

5. **进行LDSC分析**：
- 按照连锁不平衡得分回归教程进行细胞类型特异性分析（https://github.com/bulik/ldsc/wiki）。
- 使用与肠道相关的多个UKBB性状的汇总统计数据（包括非癌症疾病代码、自我报告：克罗恩病、非癌症疾病代码、自我报告：溃疡性结肠炎、非癌症疾病代码、自我报告：吸收不良/乳糜泻、BMI，以及与肠道关系不太明确的性状，包括非癌症疾病代码、自我报告：高血压和主要ICD10诊断：K02牙齿龋齿）。

6. **结果可视化和显著性检验**：
- LDSC的系数P值绘制在热图中（Fig. 6）。
- 使用Bonferroni校正（R函数p.adjust）对测试的细胞类型数量进行校正，确定显著性。
...
这段文本描述了一项研究中使用的配体-受体分析和单细胞RNA测序（scRNA-seq）与CODEX整合的具体方法和步骤，特别是初始数据集的处理方式与完整数据集的差异。

1. **初始数据集**：
- 包含来自B001、B004、B005和B006供体的样本。
- 这些样本在收集其余数据之前进行了分析。

2. **注释与分析**：
- 初始数据集的注释方式与完整数据集类似，但有以下不同：
- **软件版本**：所有分析在R v.4.0.2中进行。
- **质量控制标准**：初始数据集的质量控制阈值略有不同，没有实施RNA计数需为空滴数的三倍这一要求。
- **双胞识别**：使用doubletFinder时，pK参数设置为0.09，而不是运行paramSweep_v3。
- **归一化方法**：在上皮细胞亚群分析中没有使用scTransform，而是使用Seurat的标准归一化和缩放流程，然后运行Harmony进行整合降维，再使用Seurat的findNeighbors和FindClusters对免疫细胞、基质细胞以及肠道四个主要区域的上皮细胞进行聚类。

3. **配体-受体分析**：
- 这部分分析涉及预测，然后尝试使用分子制图（Molecular Cartography）进行验证。
- 由于涉及预测验证，无法在剩余数据集中重新进行此分析。
...
该研究对来自供体B004、B008和B009的小肠（十二指肠）和结肠（乙状结肠/降结肠）样本进行了空间转录组学分析。以下是研究的详细步骤和方法：

1. **样本制备和分子绘图分析**：
- 从同一个OCT嵌入的组织阵列中切取10µm厚的冰冻切片，放置在Resolve Biosciences提供的玻璃载片上进行分子绘图分析。
- 分子绘图分析由Resolve Biosciences团队按照他们优化的“人类结肠v1.3”协议执行，目标是分析100个感兴趣的转录本，包括63个基于配体-受体预测的基因（见补充表3）和37个用于细胞类型注释的基因（见补充表9）。

2. **细胞分割和基因计数**：
- 使用DAPI信号和cellPose（v.2.0.5）进行细胞分割，接着使用Baysor（v.0.5.1）。
- 每个细胞的基因计数进行量化，并根据面积（<50或>8000像素）和总基因计数（<2）去除细胞。
- 通过Leiden聚类（Scanpy）后的标记基因表达进行手动细胞类型注释。由于只有37个基因作为细胞类型标记，并且样本数量有限（n=6），仅识别出20种细胞类型。

3. **配体-受体验证**：
- 为了验证配体-受体修复（见补充表3），首先将细胞类型与snRNA-seq注释匹配。在152个预测中，只有58个有匹配的细胞类型。
- 比较了在预测细胞类型中的结肠和小肠之间的配体和受体表达（对数变换），使用单侧Wilcoxon秩和检验。P值经过Benjamin–Hochberg程序进行多重检验校正。总共发现15对在结肠中的表达显著高于小肠（校正后的P值<0.05；见补充表6）。
- 通过置换测试评估验证的成功率（15.5%，9/58）是否高于随机。基因标签进行了交换，并重复了相同的差异表达基因（DEG）程序10000次。

通过这些步骤，研究人员能够在特定细胞类型中验证配体-受体的关系，并比较其在结肠和小肠中的表达差异。
...
该文本描述了一个整合分析过程，该过程将snRNA-seq（单核RNA测序）细胞和CODEX（共定位分子表达）细胞进行匹配，使用了一种名为MaxFuse的算法。以下是具体的讲解：

1. **MaxFuse算法简介**：
- MaxFuse是一种用于跨不同单细胞模式匹配细胞的算法，使用线性分配来匹配细胞，包括共享和不共享的特征。
- 该算法还实施了一些信号增强步骤，如图平滑和元细胞构建，以便在弱相关的模式之间匹配细胞（例如，从RNA到蛋白质）。

2. **使用MaxFuse进行细胞匹配**：
- 尽管有许多方法可用于具有强共享信息的模式整合任务（如scRNA/scATAC），但是当涉及到蛋白质和测序模式之间的整合时，由于共享特征较弱（<60个 vs. 上千个），需要专门的方法。
- 在本研究中，MaxFuse被用于将snRNA-seq细胞与CODEX细胞进行匹配。

3. **细胞类型选择**：
- 整合过程中使用了先前注释的B细胞、T细胞、单核细胞、巨噬细胞、浆细胞、杯状细胞、内皮细胞、肠内分泌细胞、平滑肌细胞和基质细胞，而其他细胞类型由于模式间共享信息有限未被使用。

4. **共空间计算和可视化**：
- 计算了一个共享的共空间来嵌入两种模式，并使用UMAP对嵌入（前20个MaxFuse成分）进行可视化。

5. **匹配性能评估**：
- 通过细胞类型注释准确性（例如，一个CODEX浆细胞匹配到一个snRNA-seq浆细胞）作为代理来评估从RNA到蛋白质模式的单细胞匹配性能，CL和小肠匹配都达到了>90%的准确性。

6. **转录组表达转移和差异表达基因（DEGs）分析**：
- 使用单细胞级别的配对信息，将转录组表达谱转移到每个个体的CODEX细胞，并随后分析各种CODEX细胞邻域中的DEGs。
- 使用R包Seurat中的FindAllMarkers函数选择DEGs，参数设置为only.pos = TRUE, min.pct = 0.3, logfc.threshold = 0.25。显示在热图上的基因为在CL和小肠数据集中共享且调整后P值<0.05的基因。

7. **基因本体（Gene Ontology）富集分析**：
- 使用PANTHER数据库对每个细胞邻域的DEGs进行基因本体富集分析。

8. **代码和数据共享**：
- 研究者在GitHub上上传了用于执行snRNA-seq和CODEX数据集MaxFuse匹配的代码，包含数据集准备和分析功能。链接：https://github.com/shuxi-aoc/maxfuse/tree/main/Archive/hubmap_nature。
...
All of the datasets (snRNA-seq, snATAC–seq and CODEX) can be visual-
ized and accessed through a website portal (https://portal.hubmap-
consortium.org/). Our landing page links all of the raw dataset IDs
and the HuBMAP ID for this Collection is HBM692.JRZB.356 and the
DOI is https://doi.org/10.35079/HBM692.JRZB.356. Supplementary
Table 10 also lists all of the dataset IDs within the HuBMAP portal where
all raw datasets are stored that can be downloaded and also viewed in a
processed state. We provide the processed fluorescence CODEX mul-
tiplexed image stacks (https://doi.org/10.5061/dryad.76hdr7t1p and
https://doi.org/10.5061/dryad.gmsbcc2sq). We also provide processed,
quantified and annotated single-cell CODEX datasets with labelled
cell types, neighbourhoods, communities, tissue units and also pro-
tein expression at Dryad (https://doi.org/10.5061/dryad.pk0p2ngrf).
Processed snRNA-seq and snATAC-seq datasets are available at https://
doi.org/10.5061/dryad.8pk0p2ns8 and https://doi.org/10.5061/
dryad.0zpc8672f. Unprocessed snRNA-seq and snATAC–seq datasets
are available at dbGaP (phs002272.v1.p1). Source data are provided
with this paper.
### Code availability

Code for analysis of the snATAC and snRNA data is available at GitHub
(https://github.com/winstonbecker/scColonHuBMAP/), and code for
processing CODEX multiplexed imaging data (https://github.com/
nolanlab/CODEX) 8 , for clustering (https://github.com/nolanlab/vor-
tex), for transferring cell type labels with STELLAR (https://github.com/
snap-stanford/stellar), for neighbourhood analysis (https://github.
com/nolanlab/NeighborhoodCoordination), for tissue schematics
(https://github.com/nolanlab/TissueSchematics) and for MaxFuse
(https://github.com/shuxiaoc/maxfuse) is available at GitHub.