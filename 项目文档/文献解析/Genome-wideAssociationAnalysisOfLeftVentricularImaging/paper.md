...
图7c的结果显示，基因预测的高PRS组（红色）在HCM的累计发病率显著高于中等PRS组（蓝色）和低PRS组（绿色）。这表明PRS可以有效区分HCM的高风险个体。高PRS组的个体在随访期间更容易发展为HCM，而低PRS组的个体则相对较少。
...
2. **分类分层**：将所有参与者按照其PRS分为三个组：低风险组（最低三分之一）、中等风险组（中间三分之一）和高风险组（最高三分之一）。
3. **事件发生分析**：分析每组中HCM的累积发生率。使用Kaplan-Meier生存曲线来估计自参与UKB以来的HCM累积发生率。
4. **风险比较**：通过比较不同PRS分层组的累积发生率，评估PRS在预测HCM风险中的效力。

### 比喻讲解：

想象一下，你在一个庞大的车库里，有成千上万的汽车，每辆汽车都有不同的发动机配置和部件质量。你想知道哪些汽车更可能在未来发生发动机故障（类似于HCM）。

首先，你使用了一种特殊的工具（类似于全基因组关联研究），检测每辆车的所有部件，找出哪些部件的组合（遗传变异）与发动机故障（HCM）有很强的关联。

接下来，你根据这些部件的组合得出每辆车的一个评分（多基因风险评分，PRS），这个评分就像是一个风险指数，告诉你这辆车发生故障的可能性有多大。

然后，你把所有的车按照这个风险评分分成三组：低风险组（最低三分之一）、中等风险组（中间三分之一）和高风险组（最高三分之一）。

接着，你对每组车进行长期跟踪，记录每辆车在不同时间点是否发生了发动机故障（HCM）。你使用一种类似于日历的工具（Kaplan-Meier生存曲线）来绘制每组车的故障发生情况。

最后，你比较不同风险组的故障发生率，发现高风险组的车确实更容易发生故障。这就说明你的风险评分（PRS）在预测哪些车（个体）更容易发生发动机故障（HCM）方面非常有效。

通过这种方式，你可以提前识别出那些需要特别注意和维护的车（高风险个体），从而预防严重故障的发生。这一过程帮助你更好地管理车库（人群），提高整体的安全性和可靠性。
...
![](images/2024-05-31-15-41-21.png)

### 结果描述：

Fig.7d展示了使用多基因风险评分（PRS）来预测肥厚型心肌病（HCM）风险的结果。图中显示了在439,981名未进行心脏磁共振成像（CMR）的个体中，按PRS分类的不同组别的累积HCM发生率。具体来看，图中用不同颜色表示PRS的三个分类组：低风险组（绿色）、中等风险组（蓝色）和高风险组（红色）。横轴表示自参与UKB以来的年数，纵轴表示HCM的累积发生率。暗色阴影表示累积发生率的中心估计值，浅色阴影表示95%置信区间（CI）。

### 图例描述：

图7 | 按PRS分层的HCM累积发生率。c和d图展示了总计439,981名未参与CMR研究的个体，按基因预测的下隔壁LVRWT的PRS分为三组：低风险组（绿色）、中等风险组（蓝色）和高风险组（红色）。更深的色块代表累计发生率的中心估计（定义为1-Kaplan-Meier生存估计）。更浅的色块代表相应的95%置信区间。横轴表示自参与UKB以来的年数；纵轴表示HCM的累积发生率。c图表示收缩末期下隔壁LVRWT的PRS，d图表示舒张末期下隔壁LVRWT的PRS。

### 关键结论:

使用舒张末期下隔壁LVRWT的多基因风险评分（PRS）能够有效地对HCM的风险进行分层。高风险组的个体在随时间推移中显著更高的HCM发生率，表明PRS可以用于识别高风险个体。

### 分析目的：

Fig.7d的目的是评估和展示舒张末期下隔壁LVRWT的多基因风险评分（PRS）在预测肥厚型心肌病（HCM）风险中的有效性。通过将个体按PRS分类，并分析其随时间推移的HCM累积发生率，这一图例旨在验证PRS作为一种风险分层工具的准确性和实用性。
...
图7d显示了在未参与CMR队列的439,981名个体中，根据基因预测的左心室下隔壁壁厚（inferoseptal LVRWT）将人群分为三个风险组。每个风险组根据基因评分（Polygenic Risk Score, PRS）分为三个三分位数：低风险组（绿色），中等风险组（蓝色）和高风险组（红色）。较深的色调表示累积发病率的中心估计（定义为1- Kaplan-Meier生存估计），较浅的色调表示相应的95%置信区间（CI）。x轴表示自英国生物银行（UKB）注册以来的年份，y轴表示肥厚型心肌病（HCM）的累积发病率。根据收缩末期和舒张末期的基因预测下隔壁壁厚（inferoseptal LVRWT）分层。
比喻讲解：

想象你在监测一大群人，他们中的一些人有较高的风险在未来得某种心脏疾病。为了提前找出这些高风险人群，我们使用了一种特殊的方法，就像给每个人的心脏健康打分一样。这个分数是基于他们的基因信息得出的。

在这次监测中，我们将人群分成了三组：低风险组（绿色）、中等风险组（蓝色）和高风险组（红色）。我们就像给每个人贴上一个颜色标签，根据他们基因分数的高低来决定他们的标签颜色。

接下来，我们记录每组人在未来几年中患上心脏疾病的情况。这个过程就像是在看这三组人的健康状况如何随时间变化。我们使用了一种统计方法（Kaplan-Meier生存估计）来计算每组人的累积发病率，并绘制成图。

图中较深的色调代表每组人的中心估计值，这就像是每组人中实际患病的情况，而较浅的色调则表示不确定性范围（95%的置信区间），就像是在说明这个估计有多少可能是准确的。

通过这种方法，我们可以清楚地看到高风险组（红色）的人在未来几年中患上心脏疾病的几率更高，而低风险组（绿色）的几率则较低。这帮助我们提前识别出那些需要特别关注和预防的人群。
...
![](images/2024-05-31-15-43-19.png)

结果描述：

图7e展示了收缩末期下隔壁壁厚（ES-IS）多基因风险评分（PRS）在肥厚型心肌病（HCM）病例（n=420）和对照组（n=439,561）中的百分位数分布。箱线图的中央线表示中位数，箱子的顶部和底部边缘分别表示第1和第3四分位数，胡须延伸至箱体外1.5倍四分位距。使用双尾Student's t检验计算P值，星号表示统计学显著差异：*p<0.05，**p<0.01，***p<0.001。
图例描述：

图7e显示了收缩末期下隔壁壁厚（ES-IS）多基因风险评分（PRS）在肥厚型心肌病（HCM）病例和对照组中的百分位数分布。箱线图的中央线表示中位数，箱子的顶部和底部边缘分别表示第1和第3四分位数，胡须延伸至箱体外1.5倍四分位距。P值由双尾Student's t检验计算，星号表示统计学显著差异：*p<0.05，**p<0.01，***p<0.001。
关键结论：

图7e表明，肥厚型心肌病（HCM）病例的收缩末期下隔壁壁厚（ES-IS）多基因风险评分（PRS）显著高于对照组。这一发现通过统计学检验显示出显著差异，P值小于0.001。这表明ES-IS PRS在区分HCM病例和对照组方面具有显著作用。
分析目的：

图7e的目的是评估收缩末期下隔壁壁厚（ES-IS）多基因风险评分（PRS）在肥厚型心肌病（HCM）病例和对照组中的分布情况，以确定PRS是否能够有效区分HCM患者与健康个体。通过这一分析，可以验证ES-IS PRS作为识别HCM高风险个体的有效性，从而为个性化疾病预防和早期干预提供支持。
...
Fig.7e显示了使用多基因风险评分（PRS）评估基因预测的下隔壁（inferoseptal）左心室壁厚（LVRWT）在收缩末期（end systole, ES-IS）对肥厚型心肌病（HCM）风险的区分能力。具体方法如下：

1. **构建PRS**：基于每个LVRWT特征的全基因组关联研究（GWAS）结果，利用C+T（聚类+阈值）方法构建多基因风险评分（PRS），计算独立遗传变异位点的效应量。
2. **样本分类**：将参与者根据其PRS值分为三组：低风险（最低三分位数）、中等风险（第二三分位数）和高风险（最高三分位数）。
3. **风险评估**：评估PRS对LVRWT特征的区分能力，并进一步验证这些PRS在无心脏磁共振成像数据的439,981名个体中识别新发HCM的能力。
4. **统计分析**：使用风险比（HR）和95%置信区间（CI）评估不同PRS组别之间的HCM风险差异。

比喻讲解：

想象你在一个大图书馆里，有成千上万本书，每本书都有独特的编码。你需要找到某些特定的书，这些书可能会影响到一个人的健康状况。为了完成这项任务，你决定用一个特别的方法来分类和找出这些书：

1. **构建PRS（分类器）**：你首先根据以往研究的结果，确定哪些书的编码（基因变异）与特定的健康状况（LVRWT特征）相关。然后，你为每本书打分（计算每个变异位点的效应量），这些分数的总和就是你的分类器（多基因风险评分，PRS）。
2. **样本分类（分组）**：接下来，你根据分类器的分数将所有书分成三组：最低分的书归为低风险组，中等分的书归为中等风险组，最高分的书归为高风险组。
3. **风险评估（评估分类器的效果）**：然后，你评估这些分组是否能够有效地预测一个人的健康风险，就像评估分类器能否准确找到对健康有影响的书。
4. **统计分析（验证结果）**：最后，你用统计方法验证这些分类是否有效，通过比较不同组别的健康风险（HR和CI），来确定分类器的效果。

这样，通过构建和验证PRS，你就能有效地找到那些可能影响健康状况的“书”，并提前识别出高风险个体，进行预防和干预。
...
![](images/2024-05-31-15-43-09.png)

### Fig.7f描述：

#### 结果描述：

Fig.7f显示了基因预测的下隔壁（inferoseptal）左心室壁厚（LVRWT）在舒张末期（end diastole, ED-IS）多基因风险评分（PRS）在肥厚型心肌病（HCM）病例和健康对照组之间的分布。图中箱线图展示了HCM病例（n=420）和对照组（n=439,561）的PRS分布情况。

#### 图例描述：

Fig.7f | PRS在HCM病例和健康对照中的分布。箱线图显示了收缩末期下隔壁左心室壁厚（ED-IS）的多基因风险评分（PRS）在肥厚型心肌病（HCM）病例和健康对照组中的分布。对HCM病例（n=420）和对照组（n=439,561）的PRS分布进行了分析。箱线图的中心线表示中位数，上下箱线分别表示第一和第三四分位数，须线延伸至箱线边缘以外1.5倍的四分位范围。P值通过双侧学生t检验计算，*表示显著差异（*p<0.05; **p<0.01; ***p<0.001）。

#### 关键结论:

在图7f中，HCM病例的PRS显著高于健康对照组。这表明基因预测的LVRWT在舒张末期的PRS在区分HCM病例和健康对照方面具有有效性。通过统计分析得出的P值显示了两组之间的显著差异，进一步证明了这种区分能力。

#### 分析目的：

图7f的目的是评估基因预测的下隔壁左心室壁厚（LVRWT）在舒张末期的多基因风险评分（PRS）是否能够有效区分肥厚型心肌病（HCM）病例和健康对照组。通过展示PRS在两组中的分布和显著性差异，图7f验证了LVRWT-PRS在识别高风险个体方面的潜力。这对于早期筛查和预防HCM具有重要意义。
...
图7f展示了多基因风险评分（PRS）与肥厚型心肌病（HCM）之间关系的分布情况。具体来说，该图显示了在舒张末期（ED）的下隔壁（IS）LVRWT的PRS百分位数在HCM病例和对照组之间的分布差异。统计分析中，使用了两个独立的群体：一组为HCM病例（n=420），另一组为对照组（n=439,561）。箱线图显示了每个群体中PRS的中位数、四分位数和范围，使用双侧学生t检验计算P值。

### 比喻讲解：

想象你在组织一场马拉松比赛，而你的目标是预测哪些选手可能在比赛中表现突出。你手上有一份选手的训练数据，包括他们的训练时长、速度等。这些数据类似于多基因风险评分（PRS），每个选手的训练数据代表他们的基因信息。

你将所有选手按训练表现分成两组：一组是你认为可能在比赛中表现优异的选手（类似于HCM病例组），另一组是普通选手（类似于对照组）。你希望通过分析训练数据来找到一个规律，帮助你更好地预测未来比赛中的潜力选手。

你绘制了两个箱线图，分别表示这两组选手的训练数据分布。通过比较这两个箱线图，你可以看到哪些训练数据（即PRS）在表现优异的选手中更为常见。例如，你发现表现优异的选手在某个训练参数上的分数显著高于普通选手。通过这种比较，你可以得出结论：某些训练数据（或PRS）与选手的比赛表现密切相关。

同样地，图7f的箱线图展示了在舒张末期下隔壁LVRWT的PRS在HCM病例和对照组之间的分布差异。通过这种方法，我们可以看出哪些PRS在HCM病例中更为常见，从而帮助我们识别出那些具有较高HCM风险的个体。这就像在比赛前识别出有潜力的选手，进行提前关注和训练，提高他们的比赛表现一样。

*****

### Figure 7的重要结论总结：

1. **多基因风险评分（PRS）与LVRWT厚度的关联**：
- 图7a和7b显示了PRS与12个LVRWT特征（尤其是收缩末期和舒张末期的下隔壁LVRWT）之间的关联。参与者的PRS越高，其LVRWT越厚。这表明PRS可以有效区分LVRWT的厚度。

2. **PRS与肥厚型心肌病（HCM）风险的关联**：
- 图7c和7d展示了439,981名未进行CMR成像的个体中，基因预测的下隔壁LVRWT（在收缩末期和舒张末期）PRS与HCM风险的累积发生率。结果表明，基因预测的下隔壁LVRWT较高的个体具有更高的HCM风险。
- 特别是，收缩末期下隔壁LVRWT的PRS在高、中、低三个风险水平之间有显著差异，表现为高风险组个体的HCM累积发生率显著高于低风险组。

3. **PRS对HCM病例和对照组的区分能力**：
- 图7e和7f显示了收缩末期和舒张末期下隔壁LVRWT的PRS在HCM病例和对照组之间的分布差异。HCM病例的PRS显著高于对照组，这进一步证明了PRS在区分HCM高风险个体方面的有效性。
- 通过双侧学生t检验，结果表明这种差异具有统计学显著性。

### 关键结论：

- **有效的风险分层**：PRS能够有效区分LVRWT的厚度，并且可以用于识别高风险的HCM个体。这为个性化的HCM预防和早期干预提供了重要依据。
- **统计显著性**：PRS在HCM病例和对照组之间的分布差异具有统计学显著性，验证了其在预测和预防HCM中的潜在应用价值。