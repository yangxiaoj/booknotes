...
该文本描述了研究人员如何定义表达程序，以反映基因表达异质性的核心模式，并将这些程序整合成元程序（metaprograms, MPs）。以下是详细讲解：
...
2. **元程序的定义**：
- 对于每组程序，研究人员定义了一个包含50个基因的共识程序，称为元程序（MP），以反映GBM中的重复异质性模式（如图2C和S1E所示，恶性元程序在图中用粗体表示）。
- 类似地，研究人员在IDH突变型胶质瘤中定义了六个元程序（如图S2A和S2B以及表S2所示，详见STAR方法）。

这些定义和整合的元程序帮助研究人员理解GBM和IDH突变型胶质瘤中的表达异质性，为进一步的研究提供了重要的基础。
...
A) 使用 BioRender.com 创建的元程序 (MP) 生成方法方案。每个样本均由 Leiden 和 NMF 单独聚类。所有莱顿和所有所有样本中的 NMF 程序均按其基因重叠进行聚类。通过选择最常出现的基因，每个簇都被折叠成一致的 MP跨集群内的程序。
...
(C) 元程序名称和对应于 (B) 中编号的簇的所选基因的表。恶性元程序以粗体表示。
...
(A) 所有 IDH 突变样本中的所有 Leiden 和 NMF 基因程序按基因重叠（杰卡德指数）进行聚类，以导出 IDH 突变元程序。
(B) IDH 突变体样品的 MP 组成。
...
2. **元程序的定义**：
- 对于每个主题簇，研究人员就像是编写了一篇综合性的文章，包含50个关键点（基因），称为元程序（MP），这篇文章反映了大百科全书中关于该主题的主要内容和模式（如图2C和S1E所示，恶性元程序在图中用粗体表示）。
...
研究人员是如何定义和整合基因表达程序的？在GBM和IDH突变型胶质瘤中，他们分别定义了多少个元程序（MP）？
...
3. **定义元程序（MP）**：
- 对于每组程序簇，定义了一个包含50个基因的共识程序，称为元程序（MP），反映了GBM中的重复异质性模式（如图2C和S1E所示）。
- 在IDH突变型胶质瘤中，定义了六个元程序（如图S2A和S2B以及表S2所示）。
...
3. **定义元程序（MP）**：
- 对于每个主题簇，编写一篇包含50个关键点的综合性文章，称为元程序（MP），反映了大百科全书中关于该主题的主要内容和模式（如图2C和S1E所示）。
...
(D) 使用先前从顶部所示的研究中定义的基因集（列）来丰富空间元程序（行）。 富集度计算公式超几何检验（？log 10 FDR 调整的 p 值）。

#### S2 c&D&E

![](images/2024-05-16-11-38-56.png)

(C) 使用先前从顶部所示的研究中定义的基因集（行）来丰富空间元程序（列）。富集值计算公式为超几何测试；显着性由 -log 10 FDR 调整的 p 值 < 0.01 定义。
...
### 图例:
Figures 2C and 2D; Table S2; Figure S2G; Figure S2H
